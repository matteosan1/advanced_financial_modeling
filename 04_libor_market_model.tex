...
\begin{frame}{The Libor Forward Model (LFM) Framework}
	\begin{itemize}
		\item \textbf{Transition from Black-76:} While Black-76 prices individual caplets/swaptions in isolation, the LFM models the \textbf{joint dynamics} of the entire forward curve.
		\item \textbf{Market Alignment:} It is the standard for path-dependent interest rate derivatives because it is calibrated directly to market-quoted volatilities.
		\item \textbf{Mathematical Foundation:} Rates $L_i(t)$ are modeled as diffusion processes. Because a swaption depends on a basket of these rates, we cannot price them independently; we must simulate the whole vector of rates simultaneously.
	\end{itemize}
	\begin{block}{2. The Terminal Measure ($P_N$, Zero-Coupon Bond)}
	\begin{itemize}
		\item \textbf{Numeraire:} The bond maturing at the final tenor $P(t, T_N)$.
		\item \textbf{Advantage:} The drift formula is significantly simpler and more computationally stable for European Swaptions.
		\item \textbf{Property:} Only the final rate $L_{N-1}$ is a martingale.
	\end{itemize}
\end{block}
\end{frame}

\begin{frame}{Stochastic Differential Equations (SDE)}
	Under the Terminal Measure $P_N$, the dynamics for rate $L_i(t)$ are:
	\[ \frac{dL_i(t)}{L_i(t)} = \mu_i(t, L(t)) dt + \sigma_i \sum_{k=1}^N A_{ik} dZ_k(t) \]
	
	\textbf{Components of the Simulation:}
	\begin{itemize}
		\item \textbf{Diffusion:} $\sigma_i$ represents the percentage volatility.
		\item \textbf{Correlation:} $A$ is the Cholesky factor of the correlation matrix $\rho$. It ensures that if the 2Y rate jumps, the 3Y rate likely follows.
		\item \textbf{State Dependency:} The drift $\mu_i$ is not constant; it changes as the rates $L_j$ evolve, creating a "feedback loop" in the simulation.
	\end{itemize}
\end{frame}

\begin{frame}{The Terminal Measure Drift Formula}
	To prevent arbitrage, we must adjust the drift of every rate that is not the numeraire:
	
	\[ \mu_i(t, L(t)) = -\sum_{j=i+1}^{N-1} \frac{\delta_j L_j(t) \sigma_i \sigma_j \rho_{ij}}{1 + \delta_j L_j(t)} \]
	
	\textbf{Numerical Implications:}
	\begin{itemize}
		\item If you omit this term, your "Raw MC" price will be fundamentally wrong (bias).
		\item \textbf{Euler Discretization:} In Python, we calculate this sum at each time step $t$ to update $L_i(t+\Delta t)$.
		\item \textbf{The Terminal Rate:} For $i=N-1$, the sum is empty, confirming $L_{N-1}$ is a martingale.
	\end{itemize}
\end{frame}

%--- Slide 6: Control Variate Logic ---
...
\begin{itemize}
	...
	\item<3-> The \textcolor{maincolor}{joint dynamics of forward rates is not involved in the pricing of a cap}, its payoff is just a sum of caplet payoffs, i.e. marginal distributions of forward rates are enough to compute the expectation, correlation does not matter. 
...
%		\item As seen Cap/Floor can be \textcolor{maincolor}{stripped} into Caplet/Floorlet which should be priced with a different volatility each. 
...
%		\item $v_{\mathcal{T}_j}^{cap}$ is called \textcolor{maincolor}{flat volatility}, and is typically quoted for a range of strikes and expiries over liquid floating rates (e.g. 3M and 6M Euribor).
%	\end{itemize}
%\end{frame}
...
for i in range(len(ttm)-1):
  ...
  if ttm_val < cap_maturities[0]:
    ...
    P1 = cap1.npv(vol_interp((valuation_date + TimeInterval(ttm[i+1]) - valuation_date).days), 
                  dc, fc)
...
\end{frame}
	...
	%--- Slide: The Real-World Volatility Parametrization ---
	\begin{frame}{Functional Form for Instantaneous Volatility}
		Instead of using "raw" bootstrapped values, we often assume a functional form $\sigma_i(t)$ to ensure a smooth term structure:
		
		\[ \sigma_i(t) = [a(T_i - t) + b]e^{-c(T_i - t)} + d \]
		
		\begin{itemize}
			\item \textbf{Parameters $\{a, b, c, d\}$:} Calibrated to the entire Cap market simultaneously.
			\item \textbf{The "Hump":} This form captures the empirical reality that volatility often peaks for 2-3 year tenors before decaying.
			\item \textbf{Benefit:} Ensures that as time $t$ progresses in the Monte Carlo simulation, the volatility of the forward rates evolves realistically.
		\end{itemize}
	\end{frame}
	
	%--- Slide: Rebonato Weights ---
	\begin{frame}{Rebonato Weights: Linking Libor to Swaps}
	...
	\begin{frame}{Two-Factor Correlation Model}
		...
		\begin{columns}
			\begin{column}{0.5\textwidth}
				\textbf{Parameter $L$ (Level):}
				Represents the "Parallel Shift." Even for rates very far apart, correlation never drops below $L$.
			\end{column}
			\begin{column}{0.5\textwidth}
				\textbf{Parameter $\beta$ (Decay):}
				Represents the "Twist." How quickly the curve decouples as tenors diverge.
			\end{column}
		\end{columns}
		\vspace{0.3cm}
		\textbf{Calibration:} These are found by minimizing the error across a \textit{matrix} of Swaptions with different maturities and tenors.
	\end{frame}
	
	%--- Slide: The Rebonato Formula (Summary) ---
	\begin{frame}{The Calibration Target}
		...
		\begin{block}{Calibration Workflow}
			\begin{enumerate}
				\item Bootstrap/Fit $\sigma_i$ from Caplets.
				\item Calculate fixed weights $w_i$ from the initial yield curve.
				\item Solve for $\{L, \beta\}$ using a non-linear optimizer to match market Swaption volatilities.
			\end{enumerate}
		\end{block}
	\end{frame}
...
\begin{frame}{LFM Calibration: From Market to Model}
	...
	\begin{block}{The Objective Function}
		Minimize: $\sum \left( Price_{Market} - Price_{LFM}(\sigma, \rho) \right)^2$
	\end{block}
\end{frame}

%--- Slide 10: Volatility Calibration ---
...
%--- Slide 11: Correlation Calibration ---
\begin{frame}{Calibrating Correlation ($\rho$)}
	Swaption prices depend on the interaction between different forward rates. The Swap Rate variance is approximated by:
	\[ \sigma_{Swap}^2 \approx \sum_{i,j} w_i w_j \sigma_i \sigma_j \rho_{ij} \]
	
	\textbf{The Parametric Approach:}
	...
	\begin{itemize}
		...
		\item We solve for $\beta$ by minimizing the error relative to the \textbf{Swaption Matrix}.
	\end{itemize}
\end{frame}
	...
	%--- Slide 7: Finding the Optimal Beta ---
	...
	\begin{frame}{Algorithm Summary}
		\begin{enumerate}
			\item \textbf{Decompose:} Cholesky decomposition of $\rho$.
			\item \textbf{Simulate:} Step through time using Euler or Predictor-Corrector.
			\item \textbf{Calculate Drift:} Recompute $\mu_i$ at every step based on current $L$.
			\item \textbf{Payoff:} Compute Swaption payoff $Y$ and Control $X$ on each path.
			\item \textbf{Regress:} Calculate $\beta^*$ from the results and compute the final $Y_{CV}$.
		\end{enumerate}
	\end{frame}
