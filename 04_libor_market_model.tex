...
\begin{frame}{Instantaneous vs. Terminal Correlation}
	\begin{itemize}
		\item<1-> \textbf{Instantaneous Correlation ($\rho_{ij}$):} Summarizes the dependence between \textcolor{blue}{changes} (shocks) of different forward rates at time $t$.
		\begin{equation*}
			\langle dW_i, dW_j \rangle = \rho_{ij} dt
		\end{equation*}
		\item<2-> \textbf{Terminal Correlation:} Summarizes the dependence between the \textcolor{blue}{levels} of two forward rates at a future time $T_\alpha$.
		\item<3-> \textbf{The Problem:} In the LFM, the terminal correlation is \textbf{not} solely determined by $\rho_{ij}$. It is heavily influenced by the \textit{path} of the instantaneous volatilities $\sigma_i(t)$.
	\end{itemize}
...
\begin{frame}{Case Study: Why Volatility Scheduling Matters}
	Assume $F_2(0)=F_3(0)=0.05$, $\rho_{2,3}=0.75$, and $T_0 = T_1 - T_0 = 0.5$. 
	...
	\begin{center}
		\begin{tabular}{ccccc}
			\toprule
			& $\sigma_{2,1}$ & $\sigma_{3,1}$ & $\sigma_{2,2}$ & $\sigma_{3,2}$ \\ 
			\midrule
			\textbf{Case A} & \textcolor{red}{0.5} & 0.1 & 0.1 & \textcolor{red}{0.5} \\ 
			\textbf{Case B} & 0.1 & 0.1 & \textcolor{maincolor}{0.5} & \textcolor{maincolor}{0.5} \\ 
			\bottomrule
		\end{tabular}
	\end{center}
	\vspace{0.3cm}
	...
	\begin{itemize}
		\item<2-> \textbf{Case A (Low Overlap):} $F_2$ is volatile when $F_3$ is quiet. Result: $1.875 \cdot 10^{-3}$.
		\item<3-> \textbf{Case B (High Overlap):} Both rates are highly volatile simultaneously in the second period. Result: $2.343 \cdot 10^{-4}$.
	\end{itemize}
	
	\uncover<4->{
		\begin{alertblock}{Conclusion}
			The \textbf{timing} of volatility (scheduling) is a degree of freedom that must be used to calibrate to Swaptions after Caplets are matched.
		\end{alertblock}
	}
\end{frame}
...
Let's look at the numbers again. In both Case A and Case B, the Caplets are identical because the "total energy" (∫σ2) is the same for each rate. But look at how the "Rowers" (Rates) are pulling together:
Case A: The "Missed Connection"

Period 1 (T0​): F2​ is sprinting (σ=0.5), but F3​ is barely moving (σ=0.1).

Period 2 (T1​−T0​): F2​ is exhausted (σ=0.1), while F3​ starts sprinting (σ=0.5).

Terminal Result: Even though they have an instantaneous correlation of ρ=0.75, they are never "strong" at the same time. The product σ2​σ3​ is always 0.5×0.1=0.05.

Expectation: ≈1.8×10−3.

Case B: The "Power Surge"

Period 1 (T0​): Both rates are quiet (σ=0.1 and σ=0.1). The product is tiny (0.01).

Period 2 (T1​−T0​): Both rates explode together (σ=0.5 and σ=0.5).

Terminal Result: Now, that ρ=0.75 correlation is being multiplied by a massive product: 0.5×0.5=0.25.

Expectation: ≈2.3×10−4.


\begin{frame}{Forward Rates Correlation}
	\begin{itemize}
		\item Once we have calibrated the model volatilities we can move to the correlation $\rho$.
		\item In order to do so we need to link the correlation to measurable quantity. It turns out that this link can be found with the swaption volatility.
