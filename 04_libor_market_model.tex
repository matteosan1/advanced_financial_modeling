...
\textbf{Note:} we don’t know $\mathbb{E}[f(X)]$, as the reason we are doing this sampling after all is to find the expectation of $f$. However, we can construct $q$ such that it has \emph{high} density wherever $f(x)p(x)$ is \emph{high}.
\end{frame}
...
\end{enumerate}
\small{\textbf{Let's why this is this a bad idea.}}% ? Beyond knowing that $\theta$ is very small, do we even care about estimating $\theta$ accurately ?
\begin{center}
