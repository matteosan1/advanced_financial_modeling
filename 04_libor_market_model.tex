...
\begin{frame}{Setting the Model}
  ...
  \item<4-> $\bm{W}^i$ is a $n$-dimensional \textcolor{maincolor}{correlated} Brownian motion under $\mathbb{Q}^{T_i}$, with \emph{instantaneous} correlation matrix $\bm{\rho}$, i.e. $< d\bm{W}^i(t) d\bm{W}^j(t)> = \bm{\rho} dt$.
...
\begin{frame}{Functional Form for Instantaneous Volatility}
	...
	\begin{itemize}
		...
		\item This form captures the empirical reality that volatility often peaks for 1-3 year tenors before decaying (the hump).
