...
\begin{frame}{Swaption Calibration}
	...
	From the Taylor approximation we have seen before, we know:
	\begin{equation*}
		\frac{dS}{S} \approx \sum w_i \frac{dF_i}{F_i}
	\end{equation*}
	
	If we assume $dF_i/F_i=\sigma_i dW_i$, then the variance of the sum is the sum of the covariances:
	\begin{align*}
...
\begin{frame}{Rebonato Weights: Linking Libor to Swaps}
	...
	To compute a swaption volatility, we first need to express the variance of the swap rate $S_{\alpha,\beta}(t)$ in terms of the volatilities and correlations of the underlying forward rates $F_i(t)$.
	
	
	A swap rate for a period starting at $T_\alpha$ and ending at $T_\beta$ is defined as:
	\begin{equation*}
		S_{\alpha,\beta}(t) = \frac{P(t, T_\alpha)-P(t,T_\beta)}{\sum_{j=\alpha+1}^{\beta}\tau_jP(t,T_j)}
	\end{equation*}
	
	which is a non-linear function of multiple forward rates. To find how the volatility of the swap rate relates to the volatilities of the forward rates, we look at the change ($dS$, 
	Rebonato approximates at the first order)
	\begin{equation*}
		{dS} \approx \sum_{i=\alpha+1}^{\beta}\frac{\partial S}{\partial F_i}dF_i
	\end{equation*}
	
	To turn this into a relative change (which leads to volatility), we divide by $S$ 
	\begin{equation*}
		\frac{dS}{S} \approx \sum_{i=\alpha+1}^{\beta}\left(\frac{\partial S}{\partial F_i}\frac{F_i}{S}\right)\frac{dF_i}{F_i}
	\end{equation*}
	
	The term in the parentheses is your weight $w_i$. It represents the elasticity of the swap rate with respect to a specific forward rate. 
	Rebonato’s "genius" move was proving that while $F_i$ and $S$ move over time, the ratio $\frac{\partial S}{\partial F_i}\frac{F_i}{S}$ stays remarkably constant (it is "frozen"), 
	allowing us to treat it as a constant coefficient.
	
	In practice, the most common analytical form for these weights (assuming t=0 or frozen weights) is:
	\begin{equation*}
	...
	This approximation is generally excellent for "at-the-money" (ATM) swaptions but can lose accuracy for deep out-of-the-money options where the skew becomes dominant.
