...
%	\end{frame}	
	...
	\begin{frame}{Quanto Options: Motivation and Setup}
		\begin{itemize}
			\item \textbf{Definition:} A Quanto is a derivative where the underlying asset is denominated in a \textcolor{blue}{foreign currency}, but the payoff is paid in a \textcolor{red}{domestic currency} at a fixed exchange rate $X_{fix}$.
			\item \textbf{The Assets:}
			\begin{itemize}
			...
			\end{itemize}
			\item \textbf{The Problem:} The foreign asset $S_t^f$ is a martingale under the foreign risk-neutral measure $\mathbb{Q}^f$ (when divided by $B_f$). However, the domestic investor lives in the $\mathbb{Q}^d$ world.
			\item \textbf{Numeraire Change:} To price the asset in the domestic world, we must use the domestic bank account $B_d(t)$ as numeraire, but the asset's value in domestic terms is $S_t^d = X_t S_t^f$.
		\end{itemize}
	\end{frame}
...
%	\end{frame}
	...
	%	\begin{block}{Theorem (German, El Karoui and Rochet, 1995)}
		...
		%		\begin{equation}
			%			\frac{X_t}{N_t} = \E^{\mathcal{Q}^N}\left[\frac{X_T}{N_T}|\mathcal{F}_t\right]
			%		\end{equation}
		...
		%		\begin{equation}
			%			\frac{X_t}{U_t} = \E^{\mathcal{Q}^U}\left[\frac{X_T}{U_T}|\mathcal{F}_t\right]
			%		\end{equation}
		%	\end{block}
	%\end{frame}
	...
	%	\begin{equation}
		%	\Pi(0,X)=S_0(0)\E^0\left[\frac{X}{S_0(T)}\right]
		%	\end{equation}
	%	\item But also
	%	\begin{equation}
		%	\Pi(0,X)=S_1(0)\E^1\left[\frac{X}{S_1(T)}\right]
		%	\end{equation}
	...
	%	\begin{itemize}
		...
		%		\begin{equation}
			%			\Pi(0,X)=S_1(0)\E^0\left[\frac{X}{S_1(T)}L_0^1(T)\right]
			%		\end{equation}
		%		\item After some trivial manipulations
		%		\begin{equation}
			%			S_0(0)\E^0\left[\frac{X}{S_0(T)}\right]=
			%			S_1(0)\E^0\left[\frac{X}{S_1(T)}L_0^1(T)\right]
			%		\end{equation}
		%		\item Finally
		%		\begin{equation}
