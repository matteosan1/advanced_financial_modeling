...
\begin{document}
	...
	\begin{frame}{Numeraires}
		\begin{itemize}
			...
			\item<2-> The answer is through \textcolor{maincolor}{change of numeraire}.
			\item<3-> A \textcolor{maincolor}{numeraire} is any strictly positive stochastic process $N_t$ that is taken as a unit of reference when pricing an asset $S_t$
			\begin{equation*}
	...
	\begin{frame}{Numeraires}
		\begin{itemize}
			...
			\item Others might prefer commodities: 1~oz of gold could be a numeraire.
			\item In any case, once we choose a numeraire e.g. 1 USD, we determine the value of all the other assets:
			\begin{columns}
	...
	\begin{frame}{Numeraires}
		\begin{itemize}
			\item Of course, in practice there are reasons to prefer gold to other commodities e.g. corn, live cattle, or one currency with respect to another (e.g. political reasons)\ldots
			...
			\item \textbf{Basically we want to exploit this fact in our financial models.}
		\end{itemize}
	\end{frame}
	...
	\begin{frame}{Radon-Nikodym Derivative}
		\begin{itemize}
			\item Now we need to understand how to pass from a numeraire to another, and hence by a measure to another, in an arbitrage free setting (notice that until now we have implicitly assumed the bank account $B$ as our numeraire).
			\item Before formalizing how this can be done it is worth stating the following:
			\begin{itemize}
				\item given two probability measures ($\mu$ and $\nu$) formally we can indicate with
				\begin{equation*}
					\begin{gathered}
				...
				The two measures are called equivalent if and only if $\mathbb {N}_\mu= \mathbb{N}_\nu$;
				\item In other words two probability measures are said to be equivalent when they agree on which events are "available".
			\end{itemize}
		\end{itemize}
	\end{frame}
	...
	\begin{frame}{Radon-Nikodym Derivative}
		\begin{itemize}
			...
			\begin{equation}
				\expect{*}[X|\mathcal{F}_t] = \frac{\E\left[X\cfrac{d\mathbb{Q}^*}{d\mathbb{Q}}\bigg|\mathcal{F}_t\right]}{\E[\zeta_t|\mathcal{F}_t]}
				\label{eq:conditioned_expectation}
			\end{equation}
		\end{itemize}
	\end{frame}
	
	\begin{frame}{Derivation: Abstract Bayes Formula}
		To prove $ \mathbb{E}^{\mathbb{Q}}[X|\mathcal{F}_t] = \frac{\mathbb{E}^{\mathbb{P}}[X\zeta|\mathcal{F}_t]}{\zeta_t} $, check the defining property for $A \in \mathcal{F}_t$:
		\begin{align*}
			\int_A \frac{\mathbb{E}^{\mathbb{P}}[X\zeta|\mathcal{F}_t]}{\zeta_t} d\mathbb{Q} &= \int_A \frac{\mathbb{E}^{\mathbb{P}}[X\zeta|\mathcal{F}_t]}{\zeta_t} \zeta d\mathbb{P} \\
			&= \int_A \mathbb{E}^{\mathbb{P}} \left[ \frac{\mathbb{E}^{\mathbb{P}}[X\zeta|\mathcal{F}_t]}{\zeta_t} \zeta \bigg| \mathcal{F}_t \right] d\mathbb{P} \\
			&= \int_A \frac{\mathbb{E}^{\mathbb{P}}[X\zeta|\mathcal{F}_t]}{\zeta_t} \mathbb{E}^{\mathbb{P}}[\zeta|\mathcal{F}_t] d\mathbb{P} \\
			&= \int_A \mathbb{E}^{\mathbb{P}}[X\zeta|\mathcal{F}_t] d\mathbb{P} = \int_A X\zeta d\mathbb{P} = \int_A X d\mathbb{Q}
		\end{align*}
	\end{frame}
	...
	\begin{frame}[fragile]{Radon-Nikodym Derivative Example}
		...
		\begin{columns}
			\column{0.5\linewidth}
			\begin{ipython}[linewidth=0.9\linewidth]
				import numpy as np
	...
	\begin{frame}[fragile]{Radon-Nikodym Derivative Example}
		\begin{itemize}
			...
			\end{equation*}
			\item What is $\E[\zeta]$ ? Intuitively, we have that
			\begin{equation*}
				\E[\zeta]=\E\left[\frac{dQ}{dP}\right]=\int \frac{dQ}{dP} dP
			\end{equation*}
	...
	\begin{frame}{Change of Numeraire}
		\begin{block}{Theorem}
			Assume exists a numeraire $N_t$ and the associated measure $\mathbb{Q}^N$, equivalent to $\mathbb{P}$, such that the price of every traded asset $S_t$ relative to $N$ is a martingale under $\mathbb{Q}^N$
			\begin{equation*}
				\frac{S_t}{N_t} = \expect{N}\left[\frac{S_T}{N_T}\bigg|\mathcal{F}_t\right],\quad 0\le t \le T
			\end{equation*}
			Let $U$ be another arbitrary numeraire. Then \textbf{there exists a measure $\mathbb{Q}^U$, also equivalent to $\mathbb{P}$, such that the price of every traded asset $S_t$, normalized to $U$, is a martingale under $\mathbb{Q}^U$}
			\begin{equation*}
	...
	\begin{tcolorbox}[colback=gray!5, colframe=maincolor, arc=0mm, boxrule=0.5pt]
		\footnotesize
		From time to time to ease the notation I will use
		\begin{equation*}
			\expect{N}\left[\cdots \middle| \mathcal{F}_t\right] = \expectt{N}{t}\left[\cdots\right]
		\end{equation*}
	\end{tcolorbox}
	...
	\begin{frame}{Change of Numeraire (Proof part 1)}
		\begin{itemize}
			...
			\begin{equation*}
				\begin{cases}
					\expectt{N}{t}\left[\cfrac{d\mathbb{Q}^U}{d\mathbb{Q}^N}\cfrac{S_T}{U_T}\right]= \expectt{N}{t}\left[\cfrac{\cancel{U_T} N_t}{N_T U_t}\cfrac{S_T}{\cancel{U_T}}\right]=\cfrac{N_t S_t}{U_t N_t} \\
					\expectt{N}{t}\left[\cfrac{d\mathbb{Q}^U}{d\mathbb{Q}^N}\right]= \expectt{N}{t}\left[\cfrac{U_T N_t}{N_T U_t}\right] = \cfrac{N_t U_t}{U_t N_t}\quad\text{(the density process is a martingale)}
				\end{cases}
			\end{equation*}
		\end{itemize}
	\end{frame}	
...
%			\begin{equation*}
%				\expect{U}\left[\cfrac{S_T}{U_T}\bigg|\mathcal{F}_t\right]=\cfrac{\expect{N}\left[\cfrac{d\mathbb{Q}^U}{d\mathbb{Q}^N}\cfrac{S_T}{U_T}\bigg|\mathcal{F}_t\right]}{\expect{N}\left[\cfrac{d\mathbb{Q}^U}{d\mathbb{Q}^N}\bigg|\mathcal{F}_t\right]} = \cfrac{\cfrac{\cancel{N_t} S_t}{\cancel{U_t} \cancel{N_t}}}{\cfrac{\cancel{N_t} U_t}{\cancel{U_t} \cancel{N_t}}} = \cfrac{S_t}{U_t}
%			\end{equation*}
%		\end{itemize}
%		\myendproof
%	\end{frame}	
	...
	\begin{frame}{Change of Numeraire Remarks}
		...
		\begin{enumerate}
			\item find a characterization of our process by means of which we can work-out more easily the fundamental pricing formula. In particular allows to find a measure associated to the new numeraire such that \textcolor{maincolor}{the price of any asset divided by that numeraire is a martingale};
			\item give a simple rule to write (the otherwise difficult to derive) Radon-Nikodym derivative;
			\item \textcolor{maincolor}{the risk-neutral price is invariant under change of numeraire}
			\begin{equation*}
				\text{Price} = \expectt{N}{t}\left[\cfrac{N_t S_T}{N_T}\right] = \expectt{U}{t}\left[\cfrac{U_t S_T}{U_T}\right]
			\end{equation*}
		\end{enumerate}
	\end{frame}
	
	\begin{frame}{The Fundamental Identity: Why do we do this?}
		\begin{itemize}
			\item \textbf{Price Invariance:} The arbitrage-free price of a claim $V$ at time $t$ is unique. The choice of numeraire only changes the \textit{representation} of the calculation:
			\begin{equation*}
			...
			\end{equation*}
			\item \textbf{The Martingale Advantage:} Under the "standard" risk-neutral measure $\mathbb{Q}^B$, assets grow at rate $r$. This drift makes integration difficult.
			\item By switching to measure $\mathbb{Q}^N$:
			\begin{enumerate}
				\item We "normalize" the world so that $N$ is the benchmark.
	...
	\begin{frame}{A Useful Separation}
		\begin{itemize}
			...
			\item<3-> The \textcolor{maincolor}{forward measure $\mathbb{Q}^T$ (also called the $T$-measure)} is defined as the martingale measure for the numeraire process $P(\cdot,T)$, the ZCB maturing in T indeed, i.e. what we called $\mathbb{Q}^2$ in the example above.
	...
	\begin{frame}{A Useful Separation}
		\begin{itemize}
			...
			\begin{equation*}
				...
				\end{aligned}
			\end{equation*}
			\item<2-> It achieves the desired separation although under a new measure. Clearly this kind of transformations are useful when $\chi$ has known dynamics under the new measure.
		\end{itemize}
	\end{frame}
...
%	\end{frame}
	...
	\begin{frame}{The Forward Rate Under $\mathbb{Q}^T$ (Proof)}
		...
		Since $F(t;S,T)P(t,T)$ is the value of a tradable portfolio (long $1/\tau$ bonds maturing at $S$, short $1/\tau$ bonds maturing at $T$), its value divided by the numeraire $P(t,T)$ must be a $\mathbb{Q}^T$-martingale
		\begin{equation*}
	...
	\begin{frame}{The Forward Rate Under $\mathbb{Q}^T$}
		...
		Which demonstrates the initial statement and also shows that \textcolor{maincolor}{the instantaneous forward rate is a martingale under the $T$-forward measure}. Note the negative sign which represents the "decay" of the bond
	...
	\begin{frame}{The Expectation Hypothesis}
		\begin{itemize}
			...
			\end{equation*}
			has a nice connection with the \textcolor{maincolor}{expectation hypothesis of the term structure of interest rates}.
			...
			The Expectation Hypothesis: This is a crucial bridge to macroeconomics. You might add a small note that in the "real world" (P measure), this hypothesis often fails due to the Term Premium. The formula f(t,T)=ET[r(T)] holds under the forward measure by construction (no-arbitrage), but f(t,T)=EP[r(T)] is an empirical claim that usually requires a risk premium adjustment.
			
			Homework Slide: The question about "tight" monetary policy is excellent. It forces students to think about how raising r(t) (the short end) affects expectations of future inflation and growth, which are embedded in the long-term forward rates f(t,T) through the forward measure.
	...
	%The Expectations Hypothesis (EH) is a theory in finance that attempts to explain the relationship between short-term and long-term interest rates, as well as the shape of the yield curve.  It essentially posits that long-term interest rates are determined by current and expected future short-term interest rates.  Here's a breakdown:   
	%
	%Core Idea:
	%
	%The EH states that the forward rate (the interest rate agreed upon today for a future transaction) is an unbiased predictor of the future spot rate (the actual interest rate at that future time).  In simpler terms, if you want to know what the market expects short-term interest rates to be in the future, just look at the current forward rates.   
	%
	%Implications:
	%
	%Yield Curve Shape: The shape of the yield curve (a graph showing interest rates at different maturities) is determined by market expectations of future short-term interest rates.
	%
	%Upward Sloping Yield Curve (Normal): Indicates that the market expects short-term interest rates to rise in the future.   
	%Downward Sloping Yield Curve (Inverted): Suggests that the market anticipates short-term interest rates to decline.   
	%Flat Yield Curve: Implies that the market expects short-term interest rates to remain relatively stable.
	%Investment Decisions: The EH suggests that investors should be indifferent between investing in a long-term bond or a series of short-term bonds, as long as they have the same expected return.  This is because the long-term rate is just the average of the expected future short-term rates.
	%
	%Formal Expression (Simplified):
	%
	%The basic idea can be expressed as:
	%
	%Forward Rate (for period n) = Expected Future Spot Rate (for period n)
	%
	%Or, more generally:
	%
	%(1 + Long-term Rate)^n = (1 + Short-term Rate 1) * (1 + Expected Short-term Rate 2) * ... * (1 + Expected Short-term Rate n)
	%
	%Criticisms and Limitations:
	%
	%The Expectations Hypothesis is rarely considered a complete explanation of the yield curve.  It has several significant limitations:
	%
	%Risk Premium: The EH ignores the concept of a risk premium. Investors typically demand a premium (extra return) for holding long-term bonds because they are exposed to greater interest rate risk. The Liquidity Preference Theory and the Market Segmentation Theory incorporate risk premiums.   
	%Other Factors: The EH focuses solely on expectations of future short-term rates. Other factors, such as supply and demand for bonds, inflation expectations, and economic growth prospects, also influence long-term rates and the shape of the yield curve.   
	%Empirical Evidence: Empirical studies have shown that the EH does not consistently hold up in real-world markets. While expectations play a role, they are not the only, or even the most important, determinant of long-term rates.
	%Related Theories:
	%
	%Liquidity Preference Theory: Extends the EH by incorporating a liquidity premium. Investors prefer short-term, more liquid bonds and require a premium to hold less liquid long-term bonds.
	%Market Segmentation Theory: Suggests that the yield curve is shaped by the supply and demand for bonds of different maturities, driven by the specific needs and preferences of different investor groups (e.g., banks, insurance companies, pension funds).
	%In Summary:
	%
	%The Expectations Hypothesis provides a basic framework for understanding the relationship between short-term and long-term interest rates. While it highlights the importance of expectations, it is an oversimplification of reality.  Other factors, especially risk premiums and supply/demand dynamics, play a crucial role in determining the shape of the yield curve.  Therefore, while the EH is a useful starting point, it should be used in conjunction with other theories to gain a more complete understanding of interest rate behavior.   
	%
	%
	...
	%
	%Bonds 102: Understanding the Yield Curve - PIMCO
	%
	%www.pimco.com
	%
	...
	%www.usbank.com
	%
	%Term premia: models and some stylised facts - Bank for International Settlements
	%
	%www.bis.org
	%
	%
	...
	%identical. The liquidity preference hypothesis states that the expected return on strategy
	...
	%\end{frame}
	    ...
	    Radon-Nikodym: Tells you how to re-weight the outcomes (the terminal distribution). It’s like saying: "In this new world, bad outcomes are 50% less likely."
	
	Girsanov: Tells you how to change the engine (the SDE). It says: "To make those outcomes 50% less likely, you must add this specific constant to the wind (drift) every second."
	
	The Link: Without Girsanov, you have a new measure Q, but you don't know the SDE for St​ under Q. You can't run a Monte Carlo simulation without Girsanov.
	...
	\begin{frame}{Girsanov Theorem}
		\begin{block}{Theorem continued}
			...
			The condition $\E\left[\exp\left(\frac{1}{2}\int_0^t\gamma_t^2dt\right)\right]<\infty$ is a sufficient but non-necessary, and it is know as the \textcolor{maincolor}{Novikov condition}.
			    ...
			    Novikov Condition: Clarify that this ensures the RN derivative is a true martingale and not just a local martingale. If it's not a true martingale, E[dPdQ​]=1, and Q is not a probability measure.
	...
	\begin{frame}{The Vasicek Model under $\mathbb{P}$}
		\begin{itemize}
			\item Statistical analysis of historical interest rate data suggests that rates are mean-reverting. Under the physical measure $\mathbb{P}$, we model the short rate $r_t$​ as:
				...
				\end{equation*}​
			where $a$ speed of mean reversion, $b$ long-term mean level, \sigma volatility, and $W_t^{\mathbb{P}$ standard brownian motion under the real-world measure.
			\item If we use this SDE to price a bond via $\E^{\mathbb{P}}[\exp(-\int r_s ds)]$, we will get a price that is inconsistent with the market. 
			\textbf{Investors demand a premium for bearing interest rate risk}.
		\end{itemize}
	\end{frame}
	
	\begin{frame}{Market Price of Risk}
		\begin{itemize}
			\item To move to the Risk-Neutral measure $\mathbb{Q}$, we assume there exists a process $\lambda t$ called the \emph{Market Price of Risk}. 
			In the simplest Vasicek case, we assume $\lambda$ is a constant vs time.
			\item According to Girsanov’s Theorem, we define the new Brownian Motion $W_t^{\mathbb{Q}}$ as:
			\begin{equation*}
	...
	\begin{frame}{The Risk-Neutral Dynamics}
		\begin{itemize}
			...
			\end{equation*}			
			\item To maintain the elegant mean-reverting structure $dr_t=a^∗(b^∗-rt)dt+\sigma dW_t^{\mathbb{Q}}$​, we distribute the terms:
			\begin{align*}
				dr_t&=[ab−art−\sigma\lambda]dt+\sigma dW_t^{\mathbb{Q}} \\​
				dr_t&=a[(b−a\sigma\lambda)−rt]dt+\sigma dW_t^{\mathbb{Q}}​
			\end{align*}
		\end{itemize}
	\end{frame}
	
	\begin{frame}{Interpreting the New Parameters}
		\begin{itemize}
			\item Under the pricing measure $\mathbb{Q}$, the SDE is:
			\begin{equation*}
				dr_t=a^∗(b^∗−rt)dt+\sigma dW_t^{\mathbb{Q}}				
			\end{equation*}
			where the new parameters are: $a^ = a^*$: the speed of reversion remains unchanged, $b^ = b - \frac{\sigma \lambda}{a^*}$: the long-term mean is shifted.
	...
	\begin{itemize}
		\item Without Girsanov we would simply know that "prices are expectations." But we wouldn't know which interest rate process to simulate.
		\item With Girsanov we have a specific recipe. We take our historical $b$, subtract the risk adjustment $\frac{\sigma\lambda}{a}$, and we have a ready-to-use 
		SDE for Monte Carlo or the Riccati equations.
		\item The Radon-Nikodym derivative that makes this happen is:
		\begin{equation*}
		\frac{d\mathbb{P}{d\mathbb{Q}}=\exp(-frac{1}{2}\int_0^T\lambda^2 dt - \int_0^T\lambda dW_t^{\mathbb{P}})
		\end{equation*}
	\end{itemize}
\end{frame}
...
\end{frame}
	...
	Slide 1: Dynamics and Multi-Dimensional Setup
	
	To price a Quanto, we first define how the assets behave in their "natural" habitats. Under the Foreign Measure $\mathbb{Q}^f$, the assets follow:
	\begin{equation*}
	 \frac{dS_t^f}{S_t^f} = r_f dt + \sigma_S dW_{1,t}^f
 	\end{equation*}
	\begin{equation*}
		\frac{dX_t}{X_t^f} = (r_d-r_f)dt + \sigma_X dW_{2,t}^f
	\end{equation*}
	Note: $X$ is domestic/foreign. Its drift under $\mathbb{Q}^f$ is not $(r_d−r_f)$​, but this is the dynamics we will transform.
	
	The two Brownian motions have a correlation $\rho$, such that $dW_1^f dW_2^f=\rho dt$. We can write $dW_{2,t}^f=\rho dW_{1,t}^f+1−\rho^2 dZ_t^f$​, where $Z^f$ is independent of $W_1^f$​.

	Slide 2: The Radon-Nikodym Derivative
	
	The Domestic investor uses Bd​(t) as a numeraire. Any asset At​ denominated in domestic currency must be a Qd-martingale when discounted by Bd​. The domestic value of the foreign bank account is Bf​(t)Xt​. Therefore, the Radon-Nikodym derivative ξt​=dQfdQd​​Ft​​ is:
	...
	Slide 3: Applying Girsanov's Theorem
	...
	Slide 5: Mathematical Example for the Exam
	...
	Ottima scelta. Il Paradosso di Siegel è il "pezzo mancante" per capire davvero perché i tassi forward e i cambi si comportano in modo strano quando passiamo da una valuta all'altra. È la dimostrazione matematica che "la media dell'inverso non è l'inverso della media".
	
	Ecco le slide per integrare questo concetto nel tuo corso.
