...
\begin{frame}{Radon-Nikodym Derivative}
  ...
  \item Before formalize how this passage can be done it is worth stating the following:
	  ...
	  \begin{equation*}
	  	\begin{gathered}
	  	\mathcal{N}_\mu := \{A\in \Omega \mid \mu(A)=0\}\\
  	  	\mathcal{N}_\nu := \{A\in \Omega \mid \nu(A)=0\}
  	  \end{gathered}
    \end{equation*}
  	the $\mu$-null sets and the $\nu$-null sets, respectively. 
  	The two measures are called equivalent if and only $\mathcal {N}_\mu= \mathcal{N}_\nu$.
...
\begin{frame}{Intuition from Expected Value}
  ...
  \item To get a sense of what a Radon-Nikodym derivative is we can write the expected value of a generic function $F(x)$ under a measure $\mathcal{P}$, with associated density function $p(x)$ as
    \begin{equation*}
      \mathbb{E}^\mathcal{P}=\int F(x)p(x)dx
    \end{equation*}
  \item Suppose there exists a function $q(x)$, which satisfies the mathematical conditions required to be a density function. Then we can write
    \begin{equation*}
      \mathbb{E}^\mathcal{P}=\int F(x)f(x)\frac{q(x)}{q(x)}dx
    \end{equation*}
  \item If we define $\psi(x)=F(x)\frac{p(x)}{q(x)}$ the expected value can be written as 
    \begin{equation*}
      \mathbb{E}^\mathcal{P}[F(x)]=\int\psi(x)q(x)dx=\mathbb{E}^\mathcal{Q}[\psi(x)]=\mathbb{E}^\mathcal{Q}\left[F(x)\frac{p(x)}{q(x)}\right]
    \end{equation*}
  \end{itemize}
\end{frame}
...
\begin{frame}{Radon-Nikodym Derivative}
  ...
  \item The expectations corresponding to the two measures are related by
    \begin{equation*}
      \mathbb{E}^{\mathcal{Q}^*}[X] = \mathbb{E}^\mathcal{Q}\left[X\frac{d\mathcal{Q}^*}{d\mathcal{Q}}\right]= \mathbb{E}^\mathcal{Q}[X\zeta]
    \end{equation*}
	\pause
...
\begin{frame}{Change of Numeraire (Proof p.2)}
	\begin{itemize}
		\item Let's prove first this second part.
  		\item By definition of $\mathcal{Q}^N$, for any asset price $S_t$ holds
	  \begin{equation*}
	    \begin{cases} 
	      \mathbb{E}_t^N\left[\cfrac{S_T}{N_T}\right] = \cfrac{S_0}{N_0} \\
\mathbb{E}_t^U\left[\cfrac{U_0 S_T}{N_0 U_T}\right] = 	      \cfrac{U_0}{N_0}\mathbb{E}_t^U\left[\cfrac{S_T}{U_T}\right] = \cfrac{\cancel{U_0} S_0}{N_0 \cancel{U_0}} = \cfrac{S_0}{N_0}
	    \end{cases}
	  \end{equation*}
  	\item Since both equations result to $S_0/N_0$ they can be equated
  	\begin{equation*}
  	\mathbb{E}_t^N\left[\cfrac{S_T}{N_T}\right] = \mathbb{E}_t^U\left[\cfrac{U_0 S_T}{N_0 U_T}\right]
  	\end{equation*}
  	\end{itemize} 
\end{frame}
...
\begin{frame}{Change of Numeraire (Proof p.2)}
  ...
  \begin{equation*}
  	\begin{cases}
    \mathbb{E}_t^N\left[\cfrac{S_T}{N_T}\right] = \mathbb{E}_t^U\left[\cfrac{S_T}{N_T} \cfrac{d\mathcal{Q}^N}{d\mathcal{Q}^U}\right] \\
    \mathbb{E}_t^N\left[\cfrac{S_T}{N_T}\right] = \mathbb{E}_t^U\left[\cfrac{U_0 S_T}{N_0 U_T}\right]
    \end{cases}\implies
	\cfrac{\cancel{S_T}}{N_T} \cfrac{d\mathcal{Q}^N}{d\mathcal{Q}^U} = \cfrac{U_0 \cancel{S_T}}{N_0 U_T}
  \end{equation*}
...
\begin{frame}{Change of Numeraire (Proof p.1)}
  ...
  \item<2-> But 
    \begin{equation*}
      \begin{cases}
	\mathbb{E}_t^N\left[\cfrac{d\mathcal{Q}^U}{d\mathcal{Q}^N}\cfrac{S_T}{U_T}\right]= \mathbb{E}_t^N\left[\cfrac{\cancel{U_T} N_0}{N_T U_0}\cfrac{S_T}{\cancel{U_T}}\right]=\cfrac{N_0 S_t}{U_0 N_t} \\
	\mathbb{E}_t^N\left[\cfrac{d\mathcal{Q}^U}{d\mathcal{Q}^N}\right]= \mathbb{E}_t^N\left[\cfrac{U_T N_0}{N_T U_0}\right] = \cfrac{N_0 U_t}{U_0 N_t}
      \end{cases}
    \end{equation*}
  \end{itemize}
\end{frame}	
...
\begin{frame}{Change of Numeraire (Proof p.1)}
	\begin{itemize}
		...
		\begin{equation*}
			\mathbb{E}^U\left[\cfrac{S_T}{U_T}\bigg|\mathcal{F}_t\right]=\cfrac{\mathbb{E}^N\left[\cfrac{d\mathcal{Q}^U}{d\mathcal{Q}^N}\cfrac{S_T}{U_T}\bigg|\mathcal{F}_t\right]}{\mathbb{E}^N\left[\cfrac{d\mathcal{Q}^U}{d\mathcal{Q}^N}\bigg|\mathcal{F}_t\right]} = \cfrac{\cfrac{\cancel{N_0} S_t}{\cancel{U_0} \cancel{N_t}}}{\cfrac{\cancel{N_0} U_t}{\cancel{U_0} \cancel{N_t}}} = \cfrac{S_t}{U_t}
		\end{equation*}
		\item But 
		\begin{equation*}
			\begin{cases}
...
\begin{enumerate}
    ...
    \begin{equation*}
    	Price = \mathbb{E}_t^N\left[\cfrac{N_0 S_T}{N_T}\right] = \mathbb{E}_t^U\left[\cfrac{U_0 S_T}{U_T}\right]
    \end{equation*}
    \end{enumerate}
\end{frame}
...
\begin{frame}{Asset Price divided by Numeraire}
	\begin{itemize}
 	\item<1-> Let $B$ be the money bank numeraire and $\mathcal{Q}^B$ the corresponding risk-neutral measure. Also let $N$ be another numeraire (note that for what just said $N/B$ is a $\mathcal{Q}^B$-martingale). 
