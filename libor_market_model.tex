...
&= \sigma_2 dW - \cfrac{1}{2}\cfrac{1}{(F_2)^2}\sigma^2_2 F^2_2 dWdW =\\
&= \sigma_2(t) dW(t) - \cfrac{1}{2}\sigma^2_2 dt
\end{aligned}
\end{equation*}

%So we have $d ln(F_2(t)) = \sigma_2(t)dW(t) - \cfrac{1}{2}\sigma^2_2(t)dt$.
...
\ln(F_2(T)) - \ln(F_2(0)) = \int_0^T \sigma_2(t)dW(t) - \int_0^T  \cfrac{1}{2}\sigma^2_2(t)dt \\
\text{Var}[\int_0^T \sigma_2(t)dW(t) - \int_0^T  \cfrac{1}{2}\sigma^2_2(t)dt\right] = \mathbb{E}[\left(\int_0^T \sigma_2(t)dW(t) \right)\right] - 0^2 = \int_0^T \sigma^2_2(t)dt
\end{gathered}
\end{equation*}
...
Compute now the option price term $\mathbb{E}^{\mathbb{Q}^2}[(F_2(T_1) - X)^+] = [(F_2(0)e^{m+V \mathcal{N}(0,1)} - X)^+] =\int_{-\infty}^{+\infty}
(F_2(0)e^{m+Vy}-X)^+p_{\mathcal{N}(0,1)}(y)dy$

Note that $F_2(0) \exp(m + Vy) - X > 0 if and only if
\begin{equation*}
y > \cfrac{-\ln\cfrac{F_2(0)}{X}-m}{V} := \bar{y}
\end{equation*}
so that
\begin{equation*}
 $\mathbb{E}^{\mathbb{Q}^2}[(F_2(T_1) - X)^+] =\int_{-\infty}^{+\infty}(F_2(0)\exp(m+Vy)-X)p_{\mathcal{N}(0,1)}(y)dy=
\int_{\bar{y}}^{+\infty}(F_2(0)\exp(m+Vy)-X)p_{\mathcal{N}(0,1)}(y)dy = 
...
F_2(0)\cfrac{1}{\sqrt{2\pi}}\int_{\bar{y}}^{+\infty}e^{-\frac{1}{2}y^2+m+Vy} p_{\mathcal{N}(0,1)}(y)dy-X(1-\Phi(\bar{y})) = 
F_2(0)\cfrac{1}{\sqrt{2\pi}}\int_{\bar{y}}^{+\infty}e^{-\frac{1}{2}(y-V)^2 +m-\frac{1}{2}V^2} p_{\mathcal{N}(0,1)}(y)dy-X(1-\Phi(\bar{y})) = 


\end{equation*}
When including the initial discount factor P(0, T2) and the year fraction τ this
is exactly the classic market Black’s formula for the T1 − T2 caplet.
