...
\begin{frame}{Forward Rate Dynamics in LMM (Proof)}
  ...
  \item<1-> So from the Girsanov theorem:
    \begin{equation*}
      d\bm{Z}^i(t) = d\bm{Z}^{i-1}(t)+\bm{\rho}\lambda dt 
    \end{equation*}
%\Rightarrow dZ_i^i = dZ_i^{i-1}(t)+\rho^{ji}\frac{\tau_i\sigma_i(t)F_i(t)}{1+F_i(t)\tau_i} dt
  \item<2-> Applying this inductively we obtain
    \begin{equation*}
  	\begin{cases}
	k < i : dZ^i_j = dZ^k_j + \sum\limits_{h=k+1}^i \rho^{jh}\cfrac{\tau_h\sigma_h(t)F_h(t)}{1+F_h(t)\tau_h} dt;\\
	k > i : dZ^i_j = dZ^k_j - \sum\limits_{h=k+1}^i \rho^{jh}\cfrac{\tau_h\sigma_h(t)F_h(t)}{1+F_h(t)\tau_h} dt;\\
  	\end{cases}
    \end{equation*}
  \end{itemize}
...
\begin{frame}{Instantaneous and Terminal Correlation}
	...
	\end{block}
	Is this terminal correlation completely determined by the instantaneous correlations $\rho_{ij}$ between $Z_i$ and $Z_j$ ?
	%\begin{block}{Definition}
...
\item Contrary from Eq.\ref{eq:terminal_correlation_ex} it is clear that there is a dependency on the instantaneous volatility.
...
&=\mathbb{E}\left[\sum_{i=\alpha+1}^{\beta}\left[\cfrac{D(0,T_{i+1})}{P(T_i,T_{i+1})^2}-D(0,T_i)(1+\tau_{i+1}K)\right]\right] = \\
\end{aligned}
\end{equation*}
\end{itemize}
\end{frame}
