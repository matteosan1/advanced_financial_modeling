...
\begin{itemize}
\item Although there are several methods to compute swaption prices, it is common practice to use Black formula. 
...
%\item Whichever method is chosen to proce swaptions, for given values of a, b, c, d and θ we can price all the swaptions in the table, thus obtaining a table of LFM prices for swaptions corresponding to the chosen values of the LFM parameters.
%\item We can then try and change the parameters a, b, c, d and θ and consider the related Φ’s (6.28) in such a way that the LFM table approaches as much as possible, in some sense, the market swaptions table. We can, for instance, minimize the sum of the squares of the differences of the corresponding swaption prices in the two tables. 
%\item However, if the number of swaptions is large, four parameters are not sufficient for practical purposes. One then needs to consider richer parametric forms for the instantaneous volatility, such as for example
%the formulation of TABLE 5, although in this case care must be taken for
%the resulting market structures to be regular enough.
\end{itemize}
\end{frame}

\begin{frame}{LSM Calibration to Swaptions}
\begin{itemize}
\item Will instantaneous correlations be inputs or outputs in our models calibration to market quotes of (caps and) swaptions we will consider later on?
%These swaptions are priced with one of the methods we will see later on, and the model prices will depend on the correlation parameters appearing in ρ. Should we infer ρ itself from swaption market quotes or should we estimate ρ exogenously and impose it, leaving the calibration only to volatility parameters? Are the parameters in ρ inputs or outputs to the calibration?
...
\begin{equation}
\mathbb{E}_t\left[\cfrac{XD(t,S)}{P(T,S)} \right]=\mathbb{E}_t\left[\mathbb{E}_T\left[\cfrac{XD(t,T)D(T,S)}{P(T,S} \right]\right]=\mathbb{E}_t\left[\cfrac{XD(t,T)}{P(T,S}\mathbb{E}_T(D(T,S))\right]=\mathbb{E}_t\left[\cfrac{XD(t,T)}{P(T,S}P(T,S)\right]=\mathbb{E}_t(XD(t,T)),\quad (t<T<S)
%\myendproof
\end{equation}

\begin{equation}
\begin{aligned}
&\mathbb{E}\left[\sum_{i=\alpha+1}^{\beta}D(0,T_i)\tau_{i+1}(F_{i+1}(T_i-K)\right] = \\
&=\mathbb{E}\left[\sum_{i=\alpha+1}^{\beta}D(0,T_i)\left[\cfrac{1}{P(T_i,T_{i+1}}-(1+\tau_{i+1}K)\right]\right] = \\
...
It is easy to compute the expected value, since under $\mathbb{Q}^{i+1}$, $F_{i+1}$ is a martingale
\begin{equation}
...
Finally
\begin{equation}
\textbf{IAS} =\sum_{i=\alpha+1}^{\beta}\left[
P(0,T_{i+1})\left[1+2\tau_{i+1}F_{i+1}(0)+\tau_{i+1}^2F_{i+1}^2(0)\exp(v_{i+1}^2)\right]-(1+\tau_{i+1}K)P(0,T_i)\right]
\end{equation}

Contrary to the plain-vanilla case, this price depends on the volatility of forward rates through the caplet volatilities $v$. Notice however that correlations between different rates are not involved in this product. 
