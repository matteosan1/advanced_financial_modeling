...
\begin{frame}{Discrete and Continuous Random Variables}
	...
	\small{
		\textbf{Example:} when 3 coins are tossed, the number of heads obtained is the random variable, which assumes the values $\Omega=\{0,1,2,3\}$ ($\Omega$ is a countable set).}
	\newline
...
\begin{frame}{Probability Distribution}
	...
	\node[xshift=5cm,yshift=-3.7cm] (image) at (current page.center) {\includegraphics[width=20px]{images/python_logo}};
...
\begin{frame}{Stochastic Process}
	Real world data is noisy (i.e. distorted), and exhibits behaviors that cannot be described by a deterministic model (always produce same result from same inputs, e.g $f(x)=x^3+2$).

	\begin{tabular}{cl}  
		\begin{tabular}{c}
			\includegraphics[height=3cm]{images/stochastic_process}
		\end{tabular}
		& \begin{tabular}{l}
			\parbox{0.45\linewidth}{
				Need to switch to \textcolor{red}{stochastic processes} in order to model the uncertainty of the data.  
			}
		\end{tabular}  \\
	\end{tabular}
...
\begin{frame}{Stochastic Differential Equation}
	...
	\begin{tabular}{cl}  
		\begin{tabular}{c}
			\includegraphics[height=3.5cm]{images/brownian_process}
		\end{tabular}
		& \begin{tabular}{l}
			\parbox{0.45\linewidth}{
				\begin{itemize}
...
\end{itemize}	
\begin{tikzpicture}[remember picture,overlay]
	\node[xshift=6.5cm,yshift=-3.7cm] (image) at (current page.center) {\includegraphics[width=20px]{images/python_logo}};
...
\begin{frame}{Log-normality}
	...
	\begin{block}{Definition}
		...
		One of the most important properties of a log-normal distribution is that it is positive-definite (a good characteristic for stock prices).
	\end{block}
	\begin{tikzpicture}[remember picture,overlay]
		\node[xshift=5cm,yshift=-3.7cm] (image) at (current page.center) {\includegraphics[width=20px]{images/python_logo}};
...
Hence, it is mandatory to know the \textcolor{red}{proper probability distribution}.
\end{frame}
...
%
%\emph{"The probability measure of a transformed random variable. Typically this transformation is the utility function of the payoff.}
%\emph{The risk-neutral measure would be the measure corresponding to an expectation of the payoff with a linear utility."}
%\end{frame}
