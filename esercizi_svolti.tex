...
%\input{exercises/exams/exam_header}
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{color}
\usepackage{array}
...
The value of a FRA at time $t$ can be expresses as $V_t=N\tau P(t, T_2)[F(t;T_1, T_2)-K]$. Given that the stochastic bank account $B_t$ is the numeraire for the risk-neutral measure $\mathbb{Q}$, explain why it no longer appears explicitly in this valuation formula. What does this "disappearance" imply about the relationship between FRAs and the replication of interest rate risk ?
...
\begin{parts}
	\part Describe the asset swap contract for a coupon bond with coupons equal to \textbf{C} and a market price equal to $P(t)$. What will happen to the price if the spread increases, all the rest being equal ? What does it mean in term of credit quality expectation by the market for the bond's issuer ?
	\part Consider the 10-year German Bund DBR 0.5\% 2026 which is currently trading at a price of 104.58.

	Given that the 10-year EUR swap rate is 0.44\% what is the par-par asset swap spread for this bond ?
	For this exercise assume that the annuity has a value of 10.0 for simplicity.
	\part Consider the 10-year Greek Government Bond GGB 3.0\% 2026 which is currently trading at a price of 75.280. Given that the 10-year EUR swap rate is 0.440\% what is the par-par asset swap spread for this bond ?
\end{parts}

\begin{solution}
\begin{parts}
	\part The Asset Swap (AS) for a coupon bond is a package consisting of:
	\begin{itemize}
		\item Buying the risky bond at its dirty price $P(t)$.
		\item Entering a swap where you pay the bond's fixed coupons \textbf{C} and receive floating LIBOR/EURIBOR plus a spread $s$.
	...
	\begin{equation*}
		s = \frac{\text{Free Price} - \text{Market Risky Price}}{\text{AnnuityRisk}}
	\end{equation*}

	If $s$ increases (all else being equal), the Risky Price $P(t)$ must decrease.
	...
	\part First, we need to determine what the bond would be worth if we discounted its cash flows at the 10-year swap rate (0.44\%).

	Since the coupon (0.5\%) is slightly higher than the swap rate (0.44\%), the bond should trade at a premium over par in the risk-free world. Using the annuity approximation:
	\begin{equation*}
	\text{RiskFree}=Par+(C-S)\cdot A = 100+(0.5\%-0.44\%)\cdot 10 = 100.60
	\end{equation*}
	\begin{equation*}
		s=\frac{(CBP - \overline{CBP})}{A} = -0.398\%
	\end{equation*}
	...
	\part Price = 75.28, Fixed Coupon $K_{bond}=3.0\%$, Swap Rate $S_{10y}=0.44\%$, Annuity $A=10.0$.

	Calculate the Bond Price
	\begin{equation*}
		CBP=100+(3.0\%-0.44\%)\cdot 10=100+(2.56\cdot 10)=125.60
	\end{equation*}

	Solve for the Asset Swap Spread
	\begin{equation*}
		s=\frac{(125-60-75.28)}{10}=5.032\%
	\end{equation*}
	The Asset Swap Spread is +503.2 bps.

	The Bund trades at a negative spread (-39.8 bps). This means the bond is so expensive (trading at 104.58 despite a tiny coupon) that you actually "give up" yield to the swap counterparty to hold it.
	...
	%The formula $P=100+(C-S)A$ is a linear approximation of $P=\sum C\cdot P_i + 100\cdot P_n$. It is highly accurate for small differences between the coupon and the swap rate.
\end{parts}
\end{solution}

\question \textbf{Default Probability} \\
How does the bond price change with non-zero default probability for the issuer ? Write
down the formula for a zero coupon when the issuer has a flat default probability of
default pdef and a recovery rate R. How would you extend it to a fixed coupon bond ?
...
\begin{center}
	\includegraphics[width=0.8\linewidth]{images/default\_tree}
\end{center}
...
\begin{solution}
	\begin{center}
		...
		\textit{Comparison of Interest Rate Hedging Instruments}
	\end{center}
\end{solution}

\question \textbf{DV01} \\
Consider a 2-year Interest Rate Swap on a notional of 1M, paying a fixed rate of 5\% and receiving Libor rate annually. The term-structure of interest rates is flat at 5\%. Estimate numerically the DV01 of the swap.

\begin{solution}
	Calculate the initial NPV, since $K$ equals the flat market rate (5\%), the Swap is at par: $NPV_0=0$.

	Bump the curve by +1 bp, i.e. $r=5.01\%$. We use the Payer Swap perspective (Pay Fixed, Receive Floating).
	\begin{align*}
		\text{Fixed Leg} &= N\cdot K\cdot \left[\frac{1}{1+0.0501}+\frac{1}{(1+0.0501)^2}\right] \approx 92838.95 \\
		\text{Fixed Leg} &= N\cdot L\cdot \left[\frac{1}{1+0.0501}+\frac{1}{(1+0.0501)^2}\right] \approx 93024.63 \\
		NPV_{+1 \text{bp}} &\approx 93024.63-92838.95\approx 186.68
	\end{align*}

	For a par swap on a flat curve, the DV01 is approximately the Annuity times the change in rate:
	\begin{equation*}
		\text{DV}01 \approx N\cdot\sum P(0,T_i)\cdot 0.0001 \approx 1\text{M}\cdot 1.85678 \cdot 0.0001 \approx 185.68
	\end{equation*}
\end{solution}
...
\begin{parts}
	\part Delta Sensitivity analytically, that is the first derivative of caplet price w.r.t. the forward rate, using the black model to price a generic caplet;
	\part Delta Vega analytically, that is the first derivative of caplet price w.r.t. the volatility, using the black model to price a generic caplet.
\end{parts}
...
\question \textbf{Swaption Payoff}
Derive the Swaption payoff formula
\begin{equation*}
	\textbf{PSw} = N\cdot P(t, T_\alpha)\cdot \mathbb{E}^{Q}_t \left[ \left( 1 - \sum_{k=\alpha+1}^\beta c_k P(T_\alpha, T_k) \right)^+ \right]
\end{equation*}

\begin{solution}
We begin with the standard payoff of a receiver swaption at maturity $T_\alpha$. The value of the swaption at time $t$ is the discounted expectation of the sum of the differences between the forward Libor rates and the fixed strike $K$:

\begin{equation*}
	\textbf{PSw} = D(t, T_\alpha) \left( \sum_{i=\alpha+1}^\beta \tau_i \cdot P(T_\alpha, T_i) (F(T_\alpha; T_{i-1}, T_i) - K) \right)^+
\end{equation*}

The forward Libor rate $F$ at time $T_\alpha$ for the accrual period $[T_{i-1}, T_i]$ is defined by the prices of Zero-Coupon Bonds (ZCBs) as follows:
\begin{equation*}
	F(T_\alpha; T_{i-1}, T_i) = \frac{1}{\tau_i} \left( \frac{P(T_\alpha, T_{i-1})}{P(T_\alpha, T_i)} - 1 \right)
\end{equation*}

Substituting this definition into the summation within the payoff:
\begin{equation*}
	\sum_{i=\alpha+1}^\beta \tau_i \cdot P(T_\alpha, T_i) \left[ \frac{1}{\tau_i} \left( \frac{P(T_\alpha, T_{i-1})}{P(T_\alpha, T_i)} - 1 \right) - K \right]
\end{equation*}

By distributing $P(T_\alpha, T_i)$ through the brackets, the $\tau_i$ terms cancel and the bond prices simplify:
\begin{equation*}
	\sum_{i=\alpha+1}^\beta \left( P(T_\alpha, T_{i-1}) - P(T_\alpha, T_i) - K \cdot \tau_i \cdot P(T_\alpha, T_i) \right)
\end{equation*}

The first part of the sum, $\sum_{i=\alpha+1}^\beta (P(T_\alpha, T_{i-1}) - P(T_\alpha, T_i))$, is a telescoping series:
\begin{equation*}
	(P(T_\alpha, T_\alpha) - P(T_\alpha, T_{\alpha+1})) + \dots + (P(T_\alpha, T_{\beta-1}) - P(T_\alpha, T_\beta))
\end{equation*}
Since $P(T_\alpha, T_\alpha) = 1$, the sum simplifies to:
\begin{equation*}
	1 - P(T_\alpha, T_\beta)
\end{equation*}

We define the coupon coefficients $c_k$ to account for the fixed leg payments and the final principal repayment:
\begin{itemize}
	\item For $k = \alpha+1, \dots, \beta-1$, let $c_k = K \tau_k$.
	\item For the final maturity $k = \beta$, let $c_\beta = 1 + K \tau_\beta$.
\end{itemize}
The expression inside the positive part operator becomes:
\begin{equation*}
	\left( 1 - \sum_{k=\alpha+1}^\beta c_k P(T_\alpha, T_k) \right)^+
\end{equation*}

Factoring in the Notional $N$ and expressing the value at time $t$ as an expectation under the risk-neutral measure $Q$ (where $P(t, T_\alpha)$ is the discount factor to the exercise date):
\begin{equation*}
	\textbf{PSw} = N \cdot P(t, T_\alpha) \cdot \mathbb{E}^{Q}_t \left[ \left( 1 - \sum_{k=\alpha+1}^\beta c_k P(T_\alpha, T_k) \right)^+ \right]
\end{equation*}
\end{solution}

\question \textbf{Bermudan Swaption}
A pension fund holds a 3-year Bermudan Receiver Swaption on a 100 million notional. The strike rate is $K=4\%$.
...
\begin{parts}
	\part If the holder exercises at $T=1$, what is the tenor of the resulting swap ?
	\part What if they exercise at $T=2$ ?
	\part Calculate the payoff at $T=1$ if the market swap rate is 3\%.
	\part At $T=1$, if the market rate is 3\%, the holder sees an immediate payoff but also knows there is a chance the rate drops to 2\% at $T=2$. Explain qualitatively why the holder might wait instead of exercising at $T=1$.
\end{parts}

\begin{solution}
	\begin{parts}
		\part Exercise at $T=1$: the holder enters a 2-year swap (for $T=1$ to $T=3$).
		\part Exercise at $T=2$: the holder enters a 1-year swap (from $T=2$ to $T=3$).
		In a Bermudan swaption, the "tenor" of the underlying swap decreases as time passes.
		\part The Exercise Value (EV) is the present value of the difference between the strike rate ($K$) and the market swap rate ($S$), multiplied by the annuity of the remaining swap.
		Using the flat discount rate of $r=4\%$, $K=0.04$, $S=0.03$:
		\begin{align*}
		A &= 1.041 + 1.0421 = 1.8861 \\
		EV_1&=(0.04-0.03)\cdot 1.8861\cdot 100\text{M}=0.01\cdot 1.8861\cdot 100\text{M}=\$1,886,100
		\end{align*}
		\part Even though $EV_1>0$, the holder compares $EV_1$ to the Continuation Value ($CV_1$): $CV_1$ is the discounted expected value of the option to exercise at $T=2$. At $T=2$, if the rate hits 2\%, the payoff is $(0.04-0.02)=1.041\cdot100\text{M}=\$1.92\text{M}$.

		The holder might wait because the Bermudan option contains a "switch" feature. Exercising now captures a 1\% spread for 2 years, but waiting might capture a 2\% spread for 1 year. If the volatility of interest rates is high, the "time value" of waiting to see if rates hit 2\% might exceed the immediate gain of \$1,886,100.
	\end{parts}
\end{solution}

\question \textbf{Callable Coupon Bond}
An investor is looking at a 3-year Callable Bond with a 5\% annual coupon and a par value of 100. The bond is "Callable at Par" (100) at the end of Year 1 and Year 2.
\begin{itemize}
	\item Scenario A (Falling Rates): At $T=1$, the market yield for a 2-year bond drops to 3\%.
	\item Scenario B (Rising Rates): At $T=1$, the market yield for a 2-year bond rises to 7\%.
\end{itemize}
\begin{parts}
	\part Calculate the price of a standard (non-callable) 3-year bullet bond at $T=1$ under Scenario A.
	\part In Scenario A, will the issuer call the bond ? Why ? What is the maximum price an investor should be willing to pay for this bond at $T=1$ ?
	\part Sketch (or describe) the price-yield relationship for this callable bond. Why does the price "flatten out" as yields drop toward 3\% ?
\end{parts}

\begin{solution}
\begin{parts}
	\part First, we calculate the value of the remaining cash flows (Year 2 and Year 3) as a standard bond ($C= 5; $T = 100$; r= 3\%$:
	\begin{equation*}
		P=\frac{5}{1.03} + \frac{105}{1.03^2} = 4.854+98.972=103.826
	\end{equation*}
	The bond would trade at 103.83 if it were not callable.
	\part The issuer will call. She is currently paying 5\% on this bond but could issue new debt at 3\%. By calling the bond at 100, they save 2\% in interest costs.

	Since the investor knows the bond will be taken away at 100 at $T=1$, they will not pay 103.83. The market price will gravitate toward the Call Price (100).

	\part %As yields fall, the price of a normal bond rises at an increasing rate (curving upward). However, for a callable bond, as the yield approaches the coupon rate, the price "levels off" and stays near 100.

	%The upside is capped by the issuer's right to call. This creates a "concave" shape in the price-yield graph.

	While a standard bond has positive convexity, a callable bond exhibits negative convexity when yields drop below the coupon rate.

	As yields fall, the value of the issuer's embedded call option increases. Since the investor is "short" this option, the bond price decrease to offset the increasing value of the option held by the issuer.

	As the yield approaches 3\% (Scenario A), the probability of the bond being called at 100 becomes almost certain. Therefore, the market will not pay much more than 100, because any premium paid would be lost immediately upon the call.
\end{parts}
\end{solution}


\question \textbf{Change of Measure}
Calculate in details and explain the rationale of the change of measure technique from risk neutral to T-forward measure applied to the risk pricing neutral formula of a generic payoff $V$.

\begin{solution}
	In the standard Risk-Neutral Pricing formula, the price of a payoff $V_T$ is:
	\begin{equation*}
		V_0=\mathbb{E}^Q\left[e^{-\int_0^T r_s ds} V_T\right]
	\end{equation*}

	The "problem" with this formula is that the discount factor is often a stochastic variable correlated with the payoff $V_T$. Taking the expectation of a product of two correlated random variables is mathematically difficult.

	By switching to the T-Forward Measure, we use the Zero-Coupon Bond $P(t,T)$ as the numeraire. Since $P(T,T)=1$ at maturity, the discounting becomes "deterministic" under this new measure, and the $P(0,T)$ term pulls out of the expectation.

	Let $(\Omega, \mathcal{F}, \mathbb{Q})$ be the risk-neutral probability space.
	\begin{itemize}
		\item \textbf{Bank Account (Numeraire $\mathbb{Q}$):} $B_t = \exp\left( \int_0^t r_s ds \right)$
		\item \textbf{Zero-Coupon Bond (Numeraire $\mathbb{Q}^T$):} $P(t, T)$
	\end{itemize}

	The density process $\zeta_t$ for the change of measure is defined by the ratio of the two numeraires:
	\begin{equation*}
		\zeta_t = \frac{d\mathbb{Q}}{d\mathbb{Q}^T} = \frac{B_t / B_0}{P(t, T) / P(0, T)} = \frac{P(0, T) e^{\int_0^t r_s ds}}{P(t, T)}
	\end{equation*}

	The value of a generic payoff $V_T$ at time $t=0$ is given by:
	\begin{equation*}
		V_0 = \mathbb{E}^\mathbb{Q} \left[ \frac{V_T}{B_T} \right]
	\end{equation*}
	Applying the change of measure formula $\mathbb{E}^\mathbb{Q}[X] = \mathbb{E}^{\mathbb{Q}^T}[X \cdot \zeta_T]$:
	\begin{equation*}
		V_0 = \mathbb{E}^{\mathbb{Q}^T} \left[ \frac{V_T}{B_T} \cdot \zeta_T \right]
	\end{equation*}
	Substituting $\zeta_T = \frac{P(0, T) B_T}{P(T, T)}$ and noting $P(T, T) = 1$:
	\begin{equation*}
		V_0 = \mathbb{E}^{\mathbb{Q}^T} \left[ \frac{V_T}{B_T} \cdot \frac{P(0, T) B_T}{1} \right]
	\end{equation*}
	The $B_T$ terms cancel out, leaving the final $T$-forward pricing formula:
	\begin{equation*}
		V_0 = P(0, T) \mathbb{E}^{\mathbb{Q}^T} [ V_T ]
	\end{equation*}

	Under the $T$-forward measure, the current price is simply the discounted expected value of the payoff, where the discount factor $P(0, T)$ is known at time $t=0$. This decouple the interest rate risk from the payoff risk.


	To solve the expectation under the T-forward measure, you can find the probabilities (or the density) in this new measure, with the Girsanov Theorem to shift from the Risk-Neutral measure.

	To compute the integral, you need the stochastic differential equation of your underlying asset under the new measure.

	According to Girsanov's theorem, if the volatility of the bond price $P(t,T)$ is $\sigma_P$, the Brownian motion under the forward measure is related to the risk-neutral Brownian motion by:
	\begin{equation*}
		dW_t^T=dW_t-\sigma_P dt
	\end{equation*}

	The drift of your underlying asset will shift by an amount equal to the volatility of the new numeraire.

	Once you have the new drift, you have two main paths to "get the probabilities". If the underlying asset is log-normal under $\mathbb{Q}^T$ the probability density function is simply a Log-normal distribution with the adjusted drift.
	Otherwise you simulate the paths of the asset using the drift corrected for the T-forward measure with Monte Carlo simulations.
...
\question \textbf{Change of Measure}
Consider a contract paying at time $T=4$ the 4$^{th}$ power of the forward rate resetting in $T=3$ \textbf{plus} a strike $K > 0$. Time is expressed in years.
\begin{parts}
	\part Solve the following pricing problem, discussing the role of hypotheses (and tools needed and used) under which you are
	working, on a unit notional:
	\begin{equation*}
		\text{Price}(0;L(3;4);K) = \mathbb{E}^Q\left[D(0;4)\max(F^4(3;3;4)+K,0)\right]
	\end{equation*}
	with
	\begin{equation*}
		dF(t;3;4) = \sigma F(t;3;4)dW^{T^4}
	\end{equation*}
	\part Does the price depend on volatility? If the answer is yes, where would you get an estimate of $\sigma$ ?
	\part At which point in time $\sigma = 0$ in this contract ?
	\part Discuss how this pricing problem would change if the payment would be at time 3 instead of 4 (so there is a \ldots time
	lag)
\end{parts}

\begin{solution}
	\begin{parts}
		\part The payoff at $T=4$ is $X = F(3,3,4)^4 + K$. Under the $T=4$ forward measure $\mathbb{Q}^4$, the price is:
		\begin{equation*}
			\text{Price}(0) = P(0,4) \mathbb{E}^{\mathbb{Q}^4} [F(3,3,4)^4 + K]
		\end{equation*}
		Given $dF(t) = \sigma F(t) dW_t^{\mathbb{Q}^4}$, $F(T)$ is log-normally distributed:
		\begin{equation*}
			F(T) = F(0) \exp\left( -\frac{1}{2}\sigma^2 T + \sigma W_T^{\mathbb{Q}^4} \right)
		\end{equation*}
		The $n$-th moment of a log-normal variable is $\mathbb{E}[X^n] = e^{n\mu + \frac{1}{2}n^2v}$. Here $\mu = \ln F(0) - \frac{1}{2}\sigma^2 T$ and $v = \sigma^2 T$. For $n=4$ and $T=3$:
		\begin{equation*}
			\mathbb{E}^{\mathbb{Q}^4}[F(3)^4] = F(0)^4 \exp\left( \frac{4(4-1)}{2} \sigma^2 \cdot 3 \right) = F(0)^4 e^{18\sigma^2}
		\end{equation*}
		\textbf{Final Price:} $P(0,4) [F(0)^4 e^{18\sigma^2} + K]$.
		\part \textbf{Does it depend on volatility?} Yes. The convexity of the $F^4$ function means higher volatility increases the expected value.

		One would calibrate $\sigma$ to the \textbf{Caplet Market}, specifically the implied volatility of a caplet with maturity $T=3$.
		\part The uncertainty vanishes at \textbf{$t=3$}. At this point, the forward rate $F(3,3,4)$ is fixed (determined by the market). From $t=3$ to $t=4$, the payoff is a known constant, and its present value only changes due to the deterministic discount factor.
		\part If paid at $T=3$, the numeraire changes to $P(t,3)$. Under $\mathbb{Q}^3$, $F(t)$ is no longer a martingale. Using the Radon-Nikodym derivative $\frac{d\mathbb{Q}^3}{d\mathbb{Q}^4} = \frac{P(0,4)}{P(0,3)}(1+\tau F(t))$, the Girsanov's Theorem yields a drift:
		\begin{equation*}
			dF(t) = \frac{\tau \sigma^2 F(t)^2}{1 + \tau F(t)} dt + \sigma F(t) dW_t^{\mathbb{Q}^3}
		\end{equation*}
		This introduces a \textbf{convexity adjustment}. %The expected value under $\mathbb{Q}^3$ will be higher than under $\mathbb{Q}^4$ because of the positive correlation between the rate and the relative value of the $T=3$ bond.
	\end{parts}
\end{solution}

\question \textbf{Fixed Income Derivatives: Comprehensive Theory}
\begin{parts}
	\part Discuss the change of measure technique and provide an example of a possible application in option pricing theory
	\part Describe the Libor market model and explain is relation with Black Formula.
	%\part Specify under which measure is the forward rate $F(0;15;16)$ a martingale in the multicurve model.
	%\part For some combinations of the parameters, the SABR implied volatility approximation displays the so-called “smile shape”, that is a dependence from the strike of the option. What are the implications for Delta-hedging and Vega-hedging European options ?
	%\part When the volatility smile is parametrized according to the SABR model, we can quantify the exposure (and therefore devise an hedging strategy) to the risks of non-parallel movements of the volatility curve. Describe intuitively these risks.
	\part What is the mathematical relation between a discount factor $P(t;T)$ and the short-rate $r(t)$? The entire derivation is not needed, but provide $P(t;T) = f (r(t))$ considering both the cases where $r(t)$ is a generic deterministic function of time and where $r(t)$ is
a random variable (possibly described by a stochastic process).
	\part Imagine the opportunity of hedging forward contracts by means of spot contracts with the same underlying. Is there any
need to rebalance the hedging portfolio once it has been set up?
	%\part How can we obtain the parameters of the drift of a stochastic volatility process? What is going to happen to thevreplicating portfolio if the functional form of the stochastic volatility process is misspecified?
\end{parts}

\begin{solution}
	\begin{parts}
		\part The Change of Measure technique allows us to shift the drift of a stochastic process to make a specific asset-numeraire ratio a martingale.

		\textbf{Example:} in pricing a European Call option, we move from the Risk-Neutral Measure $\mathbb{Q}$ (numeraire: Bank Account $B_t$) to the $T$-Forward Measure $\mathbb{Q}^T$ (numeraire: Bond $P(t,T)$). This simplifies the calculation by removing the correlation between the stochastic interest rate and the stock price, as the exercise probability becomes the "forward" exercise probability.
		\part The LMM models the evolution of a set of forward rates $F_j(t)$. Under the $(T_{j+1})$-forward measure, the forward LIBOR $F_j(t)$ is a martingale with zero drift. If volatility is deterministic, the LMM price for a caplet reduces exactly to the \textbf{Black Formula}. Thus, LMM provides a theoretical justification for using Black's formula across an entire term structure.
		%\part In a multi-curve framework, the forward rate $F(t; 15, 16)$ is a martingale under the T=16 Forward Measure ($\mathbb{Q}^{16}$) associated with the \textbf{discount curve} numeraire $P(t, 16)$. Note that the forward rate is derived from a "forwarding curve" but must be a martingale under the "discounting" measure to prevent arbitrage.
%
%		\subsection*{4. SABR Smile: Delta and Vega Hedging}
%		The "smile" implies that volatility $\sigma$ is a function of the strike $K$ and the underlying $F$.
%		\begin{itemize}
%			\item \textbf{Delta Hedging:} You must use the \textit{SABR Delta}, which includes the "sticky-delta" or "sticky-sigma" adjustments. It accounts for how the smile shifts as the underlying moves ($\frac{\partial \text{Vol}}{\partial \text{Underlying}}$).
%			\item \textbf{Vega Hedging:} Vega is no longer a single number; you must hedge against the \textit{at-the-money} vol and the \textit{skew/smile} curvature.
%		\end{itemize}
%
%		\subsection*{5. Non-parallel Volatility Risks}
%		SABR identifies risks beyond just a parallel shift in vol (Vega):
%		\begin{itemize}
%			\item \textbf{Vanna (Skew Risk):} The risk that the slope of the smile changes (tilting).
%			\item \textbf{Volga (Curvature Risk):} The risk that the "smile" becomes more or less "convex" (fat tails).
%		\end{itemize}
		\part
		\begin{itemize}
			\item \textbf{Deterministic:} $P(t,T) = \exp\left( -\int_t^T r(s) ds \right)$
			\item \textbf{Stochastic:} $P(t,T) = \mathbb{E}^{\mathbb{Q}} \left[ \exp\left( -\int_t^T r(s) ds \right) \mid \mathcal{F}_t \right]$
		\end{itemize}
		\part \textbf{Yes}, there is a need to rebalance. Even if the underlying is the same, the \textit{basis} between the spot and forward is stochastic. Furthermore, the Delta of the forward contract relative to the spot remains 1, but the \textit{discounting} of the payoff changes over time, requiring periodic adjustment to stay "delta neutral" in present value terms.

		Assuming a constant risk-free rate $r$ and no dividends for simplicity, the forward price $F(t,T)$ at time $t$ for delivery at $T$ is:
		\begin{equation*}
			F(t,T)=S_t e^{r(T-t)}
		\end{equation*}

		The value of a long forward contract $V_t$ (with strike $K$) is:
		\begin{equation*}
			V_t=e^{-r(T-t)}(F(t,T)-K)=S_t-K e^{-r(T-t)}
		\end{equation*}

		If you calculate the Delta of the contract value $V_t$ with respect to the spot price $S_t$ it results 1. This suggests a "static" hedge (buy 1 unit of spot to hedge 1 forward).

		However, this is where the rebalancing becomes necessary due to two factors: if interest rates $r$ are not constant $
		\frac{\partial V_t}{\partial r}=K(T-t)e^{-r(T-t)}$.
		As time $t$ passes or rates fluctuate, the present value of the strike $K$ changes. To remain truly "delta-neutral" in a world where $r$ is a stochastic process, you must account for the fact that the spot price and the bond price $P(t,T)$ move differently.
		Also , even with constant $r$, if you hedge a forward by buying the spot, you have to finance that spot purchase.
		You borrow $S_t$ to buy the asset. Over time $dt$, your debt grows at the rate $r$, but your forward contract's sensitivity to the spot remains 1.
		%\part
		%\begin{itemize}
			%\item \textbf{Estimation:} Drift parameters of stochastic vol (like mean reversion speed) are obtained via \textbf{historical estimation} (Time Series) or \textbf{calibration} to the prices of exotic options (VIX options or Barrier options).
			%\item \textbf{Misspecification:} If the functional form is wrong, the replicating portfolio will accumulate a "hedging error" or "leakage." You will find that your Delta-hedged gains/losses do not sum to zero, as the realized variance will differ systematically from the model-predicted variance.
		%\end{itemize}
	\end{parts}
\end{solution}
