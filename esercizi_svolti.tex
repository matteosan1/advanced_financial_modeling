...
\begin{solution}
\begin{parts}
\part No, the market is incomplete. According to the Second Fundamental Theorem of Asset Pricing, a market is complete if and only if there exists a unique equivalent martingale measure (EMM). The existence of two distinct risk-neutral measures $\mathbb{Q}^1$ and $\mathbb{Q}^2$ implies that any convex combination $\mathbb{Q}^\lambda=\lambda \mathbb{Q}^1+(1-\lambda)\mathbb{Q}^2 (\text{for } \lambda\in[0,1])$ is also a valid risk-neutral measure. Therefore, there are infinitely many such measures, which is the hallmark of an incomplete market where not every contingent claim can be perfectly replicated.
\part In an incomplete market, there is no unique "fair" price determined by replication (arbitrage). Instead, there is an interval of arbitrage-free prices. Any price $C$ within this range is consistent with the absence of arbitrage. Because we cannot perfectly hedge the option using only the underlying stock and a bond, the "price" depends on the specific measure (or utility function) chosen by the market.
\part This relates to the Harrison-Pliska framework, which formalizes the two Fundamental Theorems of Asset Pricing: the existence of at least one risk-neutral measure $\mathbb{Q}$ is equivalent to the absence of arbitrage. Here, since both $\mathbb{Q}^1$ and $\mathbb{Q}^2$ exist, the market is arbitrage-free.

The uniqueness of that measure is equivalent to market completeness. Since uniqueness fails here, the Harrison-Pliska result tells us that there are unattainable payoffs—claims that cannot be synthesized by a trading strategy.

In practical terms, this usually means the "filtration" (the information flow) contains more "shocks" (sources of uncertainty) than there are tradable assets to neutralize them.
...
In a stochastic interest rate framework prove that the price of a Zero-Coupon Bond $P(t,T)$ is strictly greater than the exponential of the expected integrated short rate.
...
\begin{solution}
Under the risk-neutral measure $\mathbb{Q}$ (with the bank account as numeraire), the bond price is the expectation of the discount factor:

\begin{equation*}
	P(t,T) = \mathbb{E}^{\mathbb{Q}}\left[ e^{-\int_t^T r_s ds} \middle| \mathcal{F}_t \right]
\end{equation*}

By identifying $f(x)=e^{-x}$ as a strictly convex function ($f''(x)>0$), and using Jensen's Inequality (\emph{"for a convex function, the "expectation of the function" is greater than the "function of the expectation"}):

\begin{equation*}
\mathbb{E}[f(X)]\geq f(\mathbb{E}[X]) \implies \mathbb{E}^Q[e^{-X}]>e^{-\mathbb{E}^Q[X]}
\end{equation*}
with $X=\int_t^T r_s ds$ you can prove that the Bond Price is higher than the price you would get if you simply "plugged in" the average interest rate path.

Intuitively, we expect the "average" outcome of the rates to dictate the "average" price. However, a standard bond price-yield curve is not a straight line; it is "bowed" toward the origin. Because the function $f(r)=e^{-rT}$ is convex: when rates fall the price increases at an accelerating rate, when rates rise the price decreases at a decelerating rate.

Imagine a Zero-Coupon Bond with a current yield of 5\%. Suppose there is volatility in the market, and there is a 50/50 chance that rates will either move up to 7\% or down to 3\%.

Rates stay at 5\%: $P=e^{-0.05\cdot 10}\approx 60.65$

Rates are volatile $P = (e^{-0.03\cdot10} + e^{-0.07\cdot10})/2 \approx 61.87$

The "Volatile" price is higher than the "Stable" price (60.65).
The gain from the rate drop is larger than the loss from the rate rise. This "net gain" is the value added by volatility.

In stochastic calculus, use Ito's Lemma to the Bond Price as a function of the integrated short rate to show this more formally. If the short rate follows a process like $dr_t=\mu dt+\sigma dW_t$, the price of the bond $P$ will have a "drift" term that includes $\frac{1}{2}\sigma^2\frac{\partial^2 P}{\partial r^2}$.
Since there is a positive convexity, the higher the volatility, the higher the expected value of the bond.
\end{solution}
...
\begin{equation*}
	\text{Value} = N \times \frac{(L - K) \times \tau}{1 + L \times \tau} = 1,231.53
\end{equation*}
where $\tau = \frac{90}{360} = 0.25$.
\begin{itemize}
	\item numerator: $1,000,000 \times (0.06 - 0.055) \times 0.25 = 1,250$
	\item discounting: $\frac{1,250}{1 + (0.06 \times 0.25)} = \frac{1,250}{1.015}$
\end{itemize}
...
\begin{solution}
To solve this exercise, we use the no-arbitrage relationship between spot rates ($L_n$) and forward rates ($F(0;m,n)$). We assume annual discrete compounding for the US Treasury bonds.
\begin{parts}
	\part The relationship between the 1-year spot rate ($L_1$=9\%), the 2-year spot rate ($L_2$=9.5\%), and the forward rate is:
	\begin{equation*}
		(1+L_2)^2=(1+L_1)^1\cdot (1+F(0;1,2))
	\end{equation*}

	Solving for $F(0;1,2)$:
	\begin{gather*}
		1+F(0;1,2)=\frac{(1+L_2)^2}{(1+L_1)}=\frac{(1+0.095)^2}{(1+0.09)}=\frac{1.199025}{1.09}\approx 1.100023 \\
	...
	If the market forward rate were, say, 9.6\%, it would be "too low" compared to the value derived from the spot market.
	One could sell a 1-year bond and buy a 2-year bond (creating a synthetic lending at 10\% for the second year) while simultaneously entering a forward contract to borrow at 9.6\%. This would lock in a risk-free profit. In an efficient market, such opportunities are eliminated by trading, forcing the forward rate to equal the implied rate.

	\part This is the 1-year forward rate starting 2 years from now. The formula is:
	\begin{gather*}
		(1+L_3)^3=(1+L_2)^2\cdot (1+F(0;2,3)) \\
		1+F(0;2,3)=\frac{(1+L_3)^3}{(1+L_2)^2}=\frac{1.10^3}{1.095^2}=1\frac{1.331}{1.199025}\approx 1.110069 \\
		F(0;2,3) \approx 11.0069\%
	\end{gather*}

	This is the annualized 2-year forward rate starting 1 year from now. The formula is:
	\begin{gather*}
		(1+L_3)^3=(1+L_1)^1\cdot (1+F(0;1,3))^2 \\
		1+F(0;1,3)^2=1\frac{1.331}{1.091}\approx 1.221101 \\
		F(0;1,3)=\sqrt{1.221101-1} \approx 1.105034
		F(0;1,3) \approx 10.5034\%
	\end{gather*}

	The total return of an investment from year 1 to year 3 can be viewed as one 2-year period or two consecutive 1-year periods:
	\begin{equation*}
...
\question \textbf{Forward Rate Agreement} \\
If the yield curve is upward sloping, will the 6M FRA rate $K$ be higher or lower than the current 6-month EURIBOR ?
...
\begin{solution}
In a market characterized by an upward-sloping yield curve, the FRA rate $K$ (which is the forward rate $F(t; T_1, T_2)$ will be higher than the current spot 6-month EURIBOR. When the curve is normal (upward-sloping), the market anticipates higher future short-term rates.
...
\begin{itemize}
	\item A FRA is a linear instrument that can be perfectly replicated using only traded bonds ($P(t,T_1)$ and $P(t,T_2)$):
	\begin{equation*}
	\textbf{FRA} = N[K\tau P(t, S) + P(t, S) - P(t, T)]
	\end{equation*}
	 Since $B_t$ is the "path-dependent" cost, FRA's depend only on the endpoints ($T_1$ and $T_2$) and not on the specific path interest rates take in between.
	\item $B_{T}$ is a stochastic variable; by simplifying the expression, we move from a probabilistic world (expectations of unknown paths) to a market-price world (observed bond prices). This allows a trader to mark-to-market a position using only the current yield curve without needing an interest rate model.
...
\question \textbf{Forward Rate Agreement} \\
Assume that company A has agreed to pay a 6-month Libor and receive a fixed interest rate of 8\% per annum (with interests payable every six months) from the face value of 100 million. Swap is 1.25 years to expire. The interest rates for 3, 9 and 15 months are: 10\%, 10.5\% and 11\% respectively. The 6-month Libor is currently 10.2\%. Calculate the value of this swap for company A.
...
Discounting results to be: $P(0,0.25)\approx 0.9753$, $P(0,0.75)\approx 0.9243$ and $P(0,1.25)\approx 0.8715$.

$K=8\%$, $\tau=0.5$
\begin{equation*}
\text{Fixed Leg Value} = NK\cdot A = 100\cdot 0.5\cdot 0.08\cdot (0.9764+0.9278+0.8777) = 11.0844 M
\end{equation*}

The floating leg instead has a first payment $T_1$ this rate was already "fixed" 3.75 months ago (the current LIBOR of 10.2\%) so Flow1$= N\tau 0.102\cdot 0.9753 = \$4.974 M$.
In the single-curve framework, a floating rate bond is worth par (100) immediately after a payment.

For the second and third payments we use the implied Forward Rates: $N[P(0,T_1)-P(0,T_3)] = 100\cdot (0.9753-0.8715)=\$10.38M$.

The total Swap Value to Company A $\text{Fixed Leg}-\text{Floating Leg} = 11.0844-(4.974+10.38)=-\$4.2696 M$
...
If the 10-year Swap Rate is 3\% and the 10-year Government Bond yield is 2.5\%, how much is the Swap Spread ?
If the market enters a financial crisis what do you expect to happen to this spread ?
...
\begin{parts}
	...
	An increasing spread indicates a deterioration in credit quality expectations. The market is demanding a higher premium to hold the issuer's debt because the perceived probability of default has risen.
	\part Since the coupon (0.5\%) is slightly higher than the swap rate (0.44\%), the bond should trade at a premium over par in the risk-free world
	\begin{equation*}
