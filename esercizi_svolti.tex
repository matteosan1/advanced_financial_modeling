...
\begin{solution}
	\begin{parts}
		...
		Exercise at $T=2$: the holder enters a 1-year swap (from $T=2$ to $T=3$).
		...
		Using the simplified flat discount rate of $r=4\%$, $K=0.04$, $S=0.03$:
		\begin{align*}
		A &= 1.041 + 1.0421 = 1.8861 \\
		EV_1&=(0.04-0.03)\cdot 1.8861\cdot 100M=0.01\cdot 1.8861\cdot 100M=\$1,886,100
		\end{align*}
		\part Even though $EV_1>0$, the holder compares $EV_1$ to the Continuation Value ($CV_1$): $CV_1$ is the discounted expected value of the option to exercise at $T=2$. At $T=2$, if the rate hits 2\%, the payoff is $(0.04-0.02)=1.041\dot100M=\$1.92M$.

		The holder might wait because the Bermuda option contains a "switch" feature. Exercising now captures a 1\% spread for 2 years, but waiting might capture a 2\% spread for 1 year. If the volatility of interest rates is high, the "time value" of waiting to see if rates hit 2\% might exceed the immediate gain of 1,886,100.
	\end{parts}
\end{solution}
