...
Describe the asset swap contract for a coupon bond with coupons equal to \textbf{C} and a market price equal to $P(t)$. What will happen to the price if the spread increases, all the rest being equal ? What does it mean in term of credit quality expectation by the market for the bond's issuer ?
...
A pension fund holds a 3-year Bermuda Receiver Swaption on a 100 million notional. The strike rate is $K=4\%$.
...
\begin{solution}
	\begin{parts}
		...
		In a Bermuda swaption, the "tenor" of the underlying swap decreases as time passes.
...
\begin{parts}
	...
	\part In Scenario A, will the issuer call the bond ? Why ? What is the maximum price an investor should be willing to pay for this bond at $T=1$ in Scenario A ?
...
\begin{parts}
	...
	Since the investor knows the bond will be taken away at 100 at $T=1$, they will not pay 103.83. The market price will gravitate toward the Call Price (100). Maximum Price: 100 (plus any accrued interest).
...
\begin{solution}
	\begin{parts}
		...
		\part \textbf{Does it depend on volatility?} Yes. The convexity of the $F^4$ function means higher volatility increases the expected value (Jensen's Inequality). \textbf{Source of $\sigma$:} One would calibrate $\sigma$ to the \textbf{Caplet Market}, specifically the implied volatility of a caplet with maturity $T=3$.
		\part The uncertainty vanishes at \textbf{$t=3$}. At this point, the forward rate $F(3,3,4)$ is fixed (determined by the market). From $t=3$ to $t=4$, the payoff is a known constant, and its present value only changes due to the deterministic pull-to-par of the discount bond.		
		\part If paid at $T=3$, the numeraire changes to $P(t,3)$. Under $\mathbb{Q}^3$, $F(t)$ is no longer a martingale. Using the Radon-Nikodym derivative $\frac{d\mathbb{Q}^3}{d\mathbb{Q}^4} = \frac{P(0,4)}{P(0,3)}(1+\tau F(t))$, Girsanov's Theorem yields a drift:
		\begin{equation*}
...
\begin{parts}
	...
	\part What is the mathematical relation between a discount factor $P(t;T)$ and the short-rate $r(t)$? The entire derivation is not needed,
but provide $P(t;T) = f (r(t))$ considering both the cases where $r(t)$ is a generic deterministic function of time and where $r(t)$ is
a random variable (possibly described by a stochastic process).
	\part We discussed the opportunity of hedging forward contracts by means of spot contracts with the same underlying. Is there any
need to rebalance the hedging portfolio once it has been set up?
	\part How can we obtain the parameters of the drift of a stochastic volatility process? What is going to happen to the
replicating portfolio if the functional form of the stochastic volatility process is misspecified?
\end{parts}
...
%		\end{itemize}
		...
		\end{itemize}
		\part \textbf{Yes}, there is a need to rebalance. Even if the underlying is the same, the \textit{basis} between the spot and forward (the cost of carry/interest rates) is stochastic. Furthermore, the Delta of the forward contract relative to the spot remains 1, but the \textit{discounting} of the payoff changes over time, requiring periodic adjustment to stay "delta neutral" in present value terms.
		\part 
		\begin{itemize}
