...
\begin{frame}{The "Sawtooth" Behavior of FRN Prices}
	\begin{itemize}
		\item<1-> Between reset dates, the FRN price is determined by the next coupon ($\text{C}_{reset}$) already fixed:
		\begin{equation*}
			\textbf{FRN}(t) = P(t, T_{\text{next}}) \cdot (N + \text{C}_{\text{fixed}})
		\end{equation*}
		\item<1-> \textbf{Price Evolution:}
		\begin{itemize}
		...
		\end{itemize}
		\item<2-> \emph{"Why did Italian CCTs trade below par in 2011 even though they are FRNs ?"}
		\item<3-> Because the market perceived a Credit Spread over EURIBOR. The "at par" proof assumes we discount at the same rate the bond pays. If the Issuer's risk increases, the bond trades at a discount (Price $<$ Par).
...
\begin{frame}{Trading and Hedging Strategies}
	\begin{itemize}
		\item \textbf{Directional Bets (Duration Exposure)}
		\begin{itemize}
			\item \textbf{Payer Swap (Pay Fixed, Rec Float):} Effectively a "short" position. It has negative duration; you profit when rates rise because the fixed rate you pay is lower than the new market equilibrium.
			\item \textbf{Receiver Swap (Rec Fixed, Pay Float):} Effectively a "long" position. Positive duration; you profit as rates fall because your locked-in fixed coupon becomes more valuable.
		\end{itemize}

		\item \textbf{Curve Trades (Slope and Carry)}
		\begin{itemize}
