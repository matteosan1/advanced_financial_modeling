...
\begin{frame}{A Linear Derivative}
	\begin{itemize}
		...
		\item Understanding linearity is crucial for Portfolio Risk Management.
 		\begin{center}
		 	\includegraphics[width=0.55\linewidth]{images/FRA_linearity}
		 \end{center}

%		2. The Greeks: Delta and Gamma
%
%		In the world of "Greeks" (risk measures), linearity is defined by the behavior of Delta and Gamma:
%
%		Delta (Δ): For an FRA, Delta is constant. This means the sensitivity to the interest rate is the same whether the rate is at 1% or 10%.
%
%		Gamma (Γ): For an FRA, Gamma is zero. Gamma measures the change in Delta. Since the slope (Delta) of a straight line never changes, the Gamma must be zero.
%
%		In contrast, an Interest Rate Cap (an option on rates) has a non-linear payoff because it only pays out if the rate exceeds a certain level, creating a "curved" or "bent" risk profile and a non-zero Gamma.
%		3. Summary of Linear vs. Non-Linear
%		Feature	FRA (Linear)	Cap/Floor (Non-Linear)
%		Payoff Shape	Straight line	Hockey-stick / Curved
%		Risk Profile	Symmetric (Gain/Loss are equal)	Asymmetric (Limited loss, unlimited gain)
%		Gamma	Zero	Positive
%		Replication	Simple (using two bonds)	Complex (requires dynamic hedging)
%		Why this matters for University Students

%		Understanding linearity is crucial for Portfolio Risk Management. If a bank has a portfolio of 1,000 FRAs, they can simply sum up the Deltas to know their total risk. If they have a portfolio of options, they have to worry about how their risk changes as the market moves (the "convexity" or Gamma risk), which is much harder to manage.
...
\begin{homework}
	\begin{frame}{\textcolor{white}{Homework}}
		\begin{itemize}
			\item[white] If the 10-year Swap Rate is 3\% and the 10-year Government Bond yield is 2.5\%, what is the Swap Spread ?
...
\begin{frame}{Case Study: Transforming the Risk Profile}
    \begin{itemize}
        \item \textbf{The Asset}: 10Y Enel Fixed-Rate Bond at 4\% Coupon.
        \item \textbf{The Strategy}: Pay Fixed 4\% / Receive $Euribor + 1.50\%$ (ASW Spread).
    \end{itemize}
    ...
    \begin{table}[]
        ...
        \begin{tabular}{|l|l|l|}
        \hline
        \textbf{Scenario} & \textbf{Fixed-Rate Bond} & \textbf{Asset Swap (ASW)} \\ \hline
        Interest Rates $\uparrow$ & \textcolor{red}{Price Drops} (Duration) & \textbf{Hedged} (Floating) \\ \hline
        Interest Rates $\downarrow$ & \textcolor{maincolor}{Price Rises} (Duration) & \textbf{Hedged} (Floating) \\ \hline
        Credit Quality $\uparrow$ & Price Rises slightly & \textcolor{maincolor}{Spread Tightens} (Gain) \\ \hline
        Credit Quality $\downarrow$ & Price Drops & \textcolor{red}{Spread Widens} (Loss) \\ \hline
        \end{tabular}
    \end{table}
    ...
    \begin{itemize}
        ...
        \begin{enumerate}
            ...
            \item \textbf{Isolation}: The investor is left with a \textbf{Pure Credit Play} (Micro) on Enel's ability to pay the spread over Euribor.
            \item \textbf{Suitability}: Best for investors who are "Bullish" on the issuer but "Bearish" on the bond market environment.
        \end{enumerate}
    \end{itemize}
\end{frame}
%\begin{frame}{A Practicle Example}
%To explain this, let's look at a scenario involving a hypothetical 10-year Enel fixed-rate bond.
%
%Imagine you believe Enel is a rock-solid company that won't default, but you are terrified that the European Central Bank (ECB) will hike interest rates, causing bond prices to crash.
%The Scenario: The "Directional" Risk
%
%If you simply buy the Enel bond at a fixed coupon of 4%:
%
%    Credit Play: If Enel stays healthy, you get your 4%. (This is what you want).
%
%    Directional Bet: If market interest rates jump from 3% to 5%, your 4% bond is now unattractive. Its market price will drop significantly.
%
%    The Result: Even though you were right about Enel’s credit, you lost money because you were wrong about the direction of interest rates.
%
...
% FIXME METTERE PLOT ASWP VALUE IN THE MARKET
...
\begin{frame}{Hedging Credit: AS vs. CDS}
	\begin {itemize}
		...
		\item In theory, no-arbitrage considerations, $s_{AS}$ of bond with maturity $T$ must be very close to corresponding maturity CDS spread $s_{CDS}$
		\begin{itemize}
	...
	\end{itemize}
\end{frame}
% FIXME METTERE PLOT CDS vs ASWP in REAL WORLD
...
\begin{homework}
    ...
    \begin{itemize}
        ...
        \item[white] How does the bond price change with non-zero default probability for the issuer ? Write down the formula for a zero coupon formula when the issuer has a flat default probability of default $p_{\text{def}}$ and a recovery rate $R$.
    \end{itemize}
\end{frame}
\end{homework}
