...
\begin{frame}{The "Sawtooth" Behavior of FRN Prices}
	\begin{itemize}
		...
		\end{itemize}
		\item \emph{"Why did Italian CCTs trade below par in 2011 even though they are FRNs ?"}	
		\item Because the market perceived a Credit Spread over EURIBOR. The "at par" proof assumes we discount at the same rate the bond pays. If the Issuer's risk increases, the bond trades at a discount (Price < Par).
...
\begin{frame}{Single Curve Framework Paradox}
	\begin{itemize}
		...
		\end{equation*}
		\item If both legs of a Basis Swap start at $T_\alpha$ and end at $T_\beta$, their values must be identical (telescoping sum).
		\item If Leg3M=Leg6M, then the basis spread $s$ should always be zero.
		\item We need the reinvestability assumption at the same risk-free rate, and must assume that the Forward Rate is perfectly consistent with the Zero-Coupon Bond prices (i.e., $F(t;T_1,T_2)=1/\tau(P_2/P_1-1)$).
