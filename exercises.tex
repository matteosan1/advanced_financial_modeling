...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\question Suppose $B(t)$ is a standard Brownian motion. For each of the following choices of $X_t$, find an equivalent probability measure $\mathcal{Q}$ such that $X_t$ is a Brownian motion in the new measure. Assume $X_0=B_0=0$
\begin{equation*}
\begin{gathered}
dX_t = 2dt + dB_t\\
dX_t = 2dt + 6dB_t
...
\begin{solution}
\begin{enumerate}[label=(\alph*),font=\itshape]
\item Apply Itô's formula:
%First, we need to express f(t, B_t) as a function of both time and the Brownian motion B_t. We can define a new function g(t, x) such that f(t, B_t) = g(t, B_t). Now, we apply Itô's formula to g(t, B_t):
%
%dg = g_t dt + g_x dB_t + 1/2 g_{xx} d[B_t]^2
%where:
%
%g_t = ∂g/∂t is the partial derivative of g with respect to time.
%g_x = ∂g/∂x is the partial derivative of g with respect to x (evaluated at x = B_t).
%g_{xx} = ∂^2g/∂x^2 is the second partial derivative of g with respect to x (evaluated at x = B_t).
...
\fillwithlines{3cm}
\begin{solution}
Define a function $g(t, x) = \ln(x)$. Applying Itô's formula to $g(t, X_t)$:
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\question Let $W_1(t)$ and $W_2(t)$ a 2D-Brownian motion defined on a probability space $(\Omega, \mathcal{F}, \mathcal{P})$. Consider two price processes $\{S_1(t):t\geq 0\}$ and $\{S_2(t):t\geq 0\}$ with corresponding SDEs given by
\begin{equation*}
\begin{aligned}
...
\item Consider an expiration date $T$, and suppose tha interest rate is a constant $r$. Show that the market price equations have a unique solution, and determine the risk-neutral probability measure $\tilde{\mathcal{P}}$ for the process $\{S_1(t),S_2(t):0\leq t\leq T\}$.
\end{enumerate}
\fillwithlines{3cm}
\begin{solution}
\begin{enumerate}[label=(\alph*),font=\itshape]
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\question Consider the process $Z_t$ whose dynamics is defined by the following SDE $dZ_t = -\phi_tZ_tdW_t, Z_0=1$, where $W_t$ is a Brownian motion under the measure $\mathcal{P}$. For any $t\geq 0$ define the a new measure $\mathcal{Q}$ according to $d\mathcal{Q}=Z_t d\mathcal{P}$.
Prove that 
\begin{equation*}
\mathbb{E}^{\mathcal{Q}}[Z_T\log(Z_T)] = \mathbb{E}^{\mathcal{Q}}\left(\frac{1}{2}\int_0^T\phi_s^2 ds\right)
\end{equation*}
...
\begin{equation*}
\mathbb{E}^{\mathcal{P}}(Z_t\log(Z_t)) = \mathbb{E}^{\mathcal{Q}}(\log(Z_t))
\end{equation*}
