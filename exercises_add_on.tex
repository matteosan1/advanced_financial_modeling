...
\cprotEnv\begin{solution}[print=true]
\begin{ipython}
import numpy as np
...
for j in range(len(nflips)):
	for i in range(N):
		results[i, j] = np.mean(np.random.randint(0, 2, size=nflips[j]))
...
for j, f in enumerate(nflips):
	plt.hist(results[:, j], bins=np.linspace(0., 1., 20), color=colors[j], alpha=.7, 
	histtype='stepfilled', edgecolor='black', label=f'{f} flips')
plt.grid(True)
plt.legend()
plt.xlabel("Flip Mean")
plt.ylabel("Frequency")
plt.show()
\end{ipython}
By fitting the result distributions with a Gaussian it can be checked that the mean is indeed close to 0.5 (we assumed 0 head/1 tail) and that the standard deviation (a.k.a. the "error") on the mean scales as $\frac{1}{\sqrt{\text{nflips}}}$.
...
%\begin{question}[subtitle=2D Random Walk  (\texttt{python})]
...
def BM(mu, sigma, x0, T, tsteps, N):
	...
	x[0, :] = x0
	x[1:, :] = np.random.normal(size=(tsteps-1, N))*np.sqrt(dt)
...
def GBM(mu, sigma, X0, T, tsteps, N):
	...
	X[1:, :] = np.exp((mu-0.5*sigma**2)*dt+sigma*dW)
...
\cprotEnv\begin{question}%[subtitle=Geometric Brownian Motion  (\texttt{python})]
...
Is the resulting dynamics linear ? Prove it, or give a counterexample.

$$X_{n+1} = (\mu-0.5\sigma^2)\Delta t + \sigma\sqrt{\Delta t}Z$$
It is linear; because it is of the form $y = mx + q$.


Is the resulting system time-invariant ? Prove it, or give a counterexample.

No, it is not time-invariant. This is because $Z$ does vary with time.

Using the model of the process $X_n$ found above:
* derive an expression for the expectation $\mathbb{E}[X_n]$;
* derive an expression for the variance $\text{Var}[X_n]$.

Because $\mathbb{E}[Z]=0$ it implies that
...
\begin{question}[subtitle=How Unlucky is Unlucky]
...
\begin{question}[subtitle=Importance Sampling and Black-Scholes]
