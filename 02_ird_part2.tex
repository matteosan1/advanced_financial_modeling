...
\begin{itemize}
	...
	\end{equation}
	\item<1-> This is a useful measure for dealers calculating the exact P\&L generated by applying a spread (or margin) to a fixed rate away from the mid-market rate.
   \item<1-> Besides giving you the information on how a currency amount would affect the fixed rate (incorporate a fee for example), the PV01 is not useful as a risk measure. 
\end{itemize}
\end{frame}
...
\begin{itemize}
	...
	\begin{equation*}
\textbf{DV01} = \sum_j \frac{\partial \textbf{PV}}{\partial r_j} \implies 
\begin{cases}
\cfrac{\partial PV_{float}}{\partial r_i} = \cfrac{\partial}{\partial r_i}\sum_j N\tau F_j e^{-r_j t_j} = N\tau \Big[ t_i - F_i t_i - t_{i-1}\Big]e^{-r_i t_i} \\
\cfrac{\partial PV_{fix}}{\partial r_i} = \cfrac{\partial}{\partial r_i}\sum_i N\tau K e^{-r_i t_i} = - N\tau K t_i e^{-r_i t_i}
\end{cases}
	\end{equation*}
	\emph{(in the multi-curve framework DV01 would be calculated for each curve)}.
    ...
    \item<3-> A trader care mostly about DV01 (as a number or, better yet, divided into buckets) because wants to know how the P\&L changes if the curves change by 1bps.
\end{itemize}
\end{frame}
...
\begin{itemize}
	...
	\end{equation*}
	\item<1-> Then bumping $\delta r$ by $\pm 1$~bp and dividing by 2 eliminates the convexity element and very accurately approximates the DV01
	\begin{equation*}
	\textbf{DV01} = \frac{\Delta \textbf{PFS}(+1~bp)-\Delta \textbf{PFS}(-1~bp)}{2}=\frac{\partial \textbf{PFS} }{\partial r}
	\end{equation*}
	\item<2-> Another common method of calculation is to use a single bumped curve by, say, $\frac{1}{100}$th of a bp, and scale the result by 100 (less accurate, since the convexity is marginalised and not eliminated)
	\begin{equation*}
...
class Swap:
    def __init__(self, notional, fixed_rate, tau, terms, rates):
        ...
        self.K = fixed_rate
        self.tau = tau
        self.t = np.array(terms)
        self.rates = np.array(rates)

    def npv_sens(self, dr=0.0001):
        rates = tf.Variable(self.rates, name="rates")
        fixed_pv = float_pv = tf.Variable(0.0)

        with tf.GradientTape(persistent=True) as tape:
            dfs = tf.math.exp(-rates[1:]*self.t[1:])
            F = (rates[1:]*self.t[1:] - rates[:-1]*self.t[:-1])/\
                (self.t[1:]-self.t[:-1])
            fixed_pv = self.N*self.K*self.tau*tf.reduce_sum(dfs)
            float_pv = tf.reduce_sum(self.N*self.tau*F*dfs)

        fixed_sens = np.sum(dr*tape.gradient(fixed_pv, rates))
        float_sens = np.sum(dr*tape.gradient(float_pv, rates))

        npv, sens = (fixed_pv - float_pv), (fixed_sens - float_sens)
...
tau         = 1.0
terms       = [0, 1, 2, 3, 4, 5]
rates       = [0.01, 0.012, 0.013, 0.014, 0.016, 0.017]

swap = Swap(N, fixed_rate, tau, terms, rates)
price, sens = swap.npv_sens()
price_man, sens_man = swap.npv_sens_manual()

print (f"Swap price: {price:,.2f} (man.: {price_man:,.2f})")
print (f"DV01: {sens:,.2f} (man.: {sens_man:,.2f})")
\end{ipython}
\begin{ioutput}

Swap price: -9,097.43 (man.: -9,097.43)
DV01: -472.60 (man.: -472.60)
\end{ioutput}
\end{columns}
\end{frame}
...
\begin{itemize}
	...
	\item<2-> Nevertheless it can be simplified by writing it in a different way...
	%		\begin{equation}
...
\begin{itemize}
	...
	\item<1-> Hence a Swaption can be regarded as an \textcolor{red}{option to exchange fixed for floating} bonds (or vice versa): a \textbf{Bond Option}.
	\item<2-> In case it would be possible to get an expression for the price of such contract we could use it to price Swaptions.
	\item <2-> It would be even simpler if we could express a Bond Option as a \emph{portfolio} of Zero Coupon Bond Options.
	\item<3-> Indeed it can be done thanks to a recipe known as \textcolor{red}{Jamshidian's decomposition}.
\end{itemize}
\end{frame}
...
\begin{frame}{Back to Bond Options}
\begin{itemize}
	...
	\item<2-> Let $t\leq T_1$, the bond payoff is given by
	\begin{equation*}
	...
	\begin{equation*}
		\textbf{CBP}=(K-\textbf{CB}(t,\mathcal{C},\mathcal{T}))^+ = \left(K-\sum_{i=1}^n c_i \Pi(t, T_i, r(t))\right)^+
	\end{equation*}
\end{itemize}
\end{frame}
...
def longstaff_schwartz(S0, r, sigma, K, tsteps, T, N, side):
    dt = T / tsteps
    df = np.exp(-r*dt)
...
V0 = longstaff_schwartz(S0, r, sigma, K, M, T, I, -1)
