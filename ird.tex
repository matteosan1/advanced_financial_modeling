...
\begin{itemize}
	\item The \emph{swap curve} is a graphical representation of the relationship between the fixed interest rate and the maturity of of interest rate swaps (typically ranging from 1 to 30 years).
	...
	\begin{columns}
		...
		\begin{center}
			\includegraphics[width=1.0\linewidth]{sde\_simulations}
		\end{center}
...
\begin{itemize}
\item The yield curve reflects the yields of various fixed-income securities, including government bonds, corporate bonds, and mortgage-backed securities.
...
\item The swap curve reflects exchanged rates over various time horizons. It offers a more accurate representation of market expectations and risk perceptions, as it is based on actual transactions.
%\item When interest rates are expected to rise, the swap curve tends to steepen as longer-term rates increase at a faster pace than shorter-term rates. Conversely, during periods of economic uncertainty, the swap curve may flatten as investors seek the safety of shorter-term instruments.
\end{itemize}
\begin{center}
	\includegraphics[width=0.4\linewidth]{sde\_simulations}
\end{center}
\end{frame}
...
\item When evaluating the swap curve, market participants should compare the benefits and drawbacks of floating and fixed rates. \item Floating rates (such as those linked to LIBOR or EURIBOR), can provide flexibility and potentially lower costs, as they reflect the prevailing market conditions. 
...
\item Considering these options, market participants may find that a floating rate position is more advantageous if they anticipate a downward sloping swap curve. This would allow them to benefit from lower fixed interest payments over time. 
\item On the other hand, if market expectations suggest an upward sloping swap curve, a fixed rate position may be more suitable to lock in a higher fixed interest rate.
...
%
%5. Utilizing Options: Options can also be utilized to manage swap curve risks effectively. By purchasing interest rate options, investors can protect themselves against adverse movements in the swap curve while still benefiting from favorable changes. For instance, an investor can buy a put option on a plain vanilla swap, which gives them the right to sell the swap at a specified price if the swap curve moves unfavorably. This provides downside protection while allowing for potential upside gains.
...
\begin{itemize}
	...
	= \sum_i \left(K_i - f_i(W)\right)^+\quad\quad\quad\quad\qedsymbol
	\end{equation*}
	}
	\column{0.35\linewidth}
	\uncover<1->{
